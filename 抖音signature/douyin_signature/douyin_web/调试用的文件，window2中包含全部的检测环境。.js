const {JSDOM} = require('jsdom');
const jsdom = new JSDOM('<!doctype html><html><body></body></html>', {
    url: "https://www.douyin.com/user/MS4wLjABAAAAYsGlaAlC63EwxXFJX-DkIdIcMNr5PxZy3h3PvIGobBFp8S11I-0xQMQ2i-Z182vk"
});
const window = jsdom.window;

window.fetch = function () {
    // console.log(arguments);
}

new window.XMLHttpRequest().__proto__.send = function () {
    console.log("https://www.douyin.com"+this._byted_url);
    debugger;
}

var location = {
    "ancestorOrigins": {},
    "href": "https://www.douyin.com/user/MS4wLjABAAAAYsGlaAlC63EwxXFJX-DkIdIcMNr5PxZy3h3PvIGobBFp8S11I-0xQMQ2i-Z182vk",
    "origin": "https://www.douyin.com",
    "protocol": "https:",
    "host": "www.douyin.com",
    "hostname": "www.douyin.com",
    "port": "",
    "pathname": "/user/MS4wLjABAAAAYsGlaAlC63EwxXFJX-DkIdIcMNr5PxZy3h3PvIGobBFp8S11I-0xQMQ2i-Z182vk",
    "search": "",
    "hash": ""
}

var document = {
    "location": location,
    "cookie": "",
    "referrer": "",
    "addEventListener": function (a, b, c, d, e, f, g) {
        // console.log(arguments);
        // debugger;
    },
    "createElement": function (a, b, c, d, e, f, g) {
        if (a == 'canvas') {
            return {
                "getContext": function (a, b, c, d, e, f, g) {
                    if (a == "2d") {
                        return {
                            "fillRect": function (a, b, c, d, e, f, g) {

                            }, "fillText": function (a, b, c, d, e, f, g) {

                            }
                        }
                    }
                },
                "toDataURL": function () {
                    return "****************************************************************"
                }
            }
        } else {
            debugger;
        }
    },
    "createEvent": function (a, b, c, d, e, f, g) {
        // console.log(arguments);
        debugger;
    }
}

var window2 = {
    "Object": Object,
    "TypeError": TypeError,
    "document": document,
    "Date": Date,
    "Math": Math,
    "navigator": {
        "plugins": {
            "0": {
                "length": 1,
                "item": function (a, b, c, d, e, f, g) {
                    // console.log(arguments);
                    debugger;
                },
                "name": "Chrome PDF Plugin",
                "filename": "internal-pdf-viewer"
            },
            "1": {
                "length": 1,
                "item": function (a, b, c, d, e, f, g) {
                    // console.log(arguments);
                    debugger;
                },
                "name": "Chrome PDF Viewer",
                "filename": "mhjfbmdgcfjbbpaeojofohoefgiehjai"
            },
            "2": {
                "length": 2,
                "item": function (a, b, c, d, e, f, g) {
                    // console.log(arguments);
                    debugger;
                },
                "name": "Native Client",
                "filename": "internal-nacl-plugin"
            }
        },
        "webdriver": false,
        "userAgent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.114 Safari/537.36",
        "platform": "Win32"
    },
    "location": location,
    "history": {},
    "Image": function (a, b, c, d, e, f, g) {
        // console.log(arguments);
        debugger;
    },
    "console": console,
    "PluginArray": function (a, b, c, d, e, f, g) {
        // console.log(arguments);
        debugger;
    },
    "indexedDB": {},
    "DOMException": function (a, b, c, d, e, f, g) {
        // console.log(arguments);
        debugger;
    },
    "parseInt": function (a, b, c, d, e, f, g) {
        // console.log(arguments);
        debugger;
    },
    "String": String,
    "Array": Array,
    "Error": Error,
    "JSON": JSON,
    "Promise": Promise,
    "WebSocket": function (a, b, c, d, e, f, g) {
        // console.log(arguments);
        debugger;
    },
    "eval": function (a, b, c, d, e, f, g) {
        // console.log(arguments);
        debugger;
    },
    "encodeURIComponent": encodeURIComponent,
    "encodeURI": encodeURI,
    "Request": function (a, b, c, d, e, f, g) {
        // console.log(arguments);
        debugger;
    },
    "Headers": function (a, b, c, d, e, f, g) {
        // console.log(arguments);
        debugger;
    },
    "decodeURIComponent": decodeURIComponent,
    "RegExp": RegExp,
    "sessionStorage": {
        "getItem": function (a, b, c, d, e, f, g) {
            // console.log(arguments);
            return null;
        },
        "removeItem": function (a, b, c, d, e, f, g) {
            // console.log(arguments);
            return null;
        }
    },
    "localStorage": {
        "removeItem": function (a, b, c, d, e, f, g) {
            console.log(a, b, c, d, e, f, g);
            delete this[a];
        },
        "getItem": function (a, b, c, d, e, f, g) {
            console.log(a, b, c, d, e, f, g);
            return this[a];
        },
        "setItem": function (a, b, c, d, e, f, g) {
            console.log(a, b, c, d, e, f, g);
            this[a] = b;
        }
    },
    "Reflect": Reflect,
    "Proxy": function (a, b, c, d, e, f, g) {
        // console.log(arguments);
        debugger;
    },
    "XMLHttpRequest": function (a, b, c, d, e, f, g) {
        // console.log(arguments);
        debugger;
    },
    "fetch": function (a, b, c, d, e, f, g) {
        // console.log(arguments);
        debugger;
    },
    "toString": Proxy,
    "HTMLElement": function (a, b, c, d, e, f, g) {
        // console.log(arguments);
        debugger;
    },
    "chrome": {
        "runtime": {
            "connect": function (a, b, c, d, e, f, g) {
                // console.log(arguments);
                debugger;
            }
        }
    }
}

// todo
// Date.prototype.getTime = function () {
//     return 1624980376000;
// }

Object.keys(window).forEach(property => {
    try {
        if (typeof global[property] === 'undefined') {
            global[property] = window[property];
        }
    } catch (e) {
        // console.log(e);
    }
});

window.sessionStorage;

window.window = window;
global.window = window

with (window) {
    var glb;
    (glb = "undefined" == typeof window ? global : window)._$jsvmprt = function (b, e, f) {
        function a() {
            if ("undefined" == typeof Reflect || !Reflect.construct) {
                return !1;
            }

            if (Reflect.construct.sham) {
                return !1;
            }

            if ("function" == typeof Proxy) {
                return !0;
            }

            try {
                Date.prototype.toString.call(Reflect.construct(Date, [], function () {
                }));
                return !0;
            } catch (b) {
                return !1;
            }
        }

        function d(b, e, f) {
            return (d = a() ? Reflect.construct : function (b, e, f) {
                var a = [null];
                a.push.apply(a, e);
                var d = new (Function.bind.apply(b, a))();

                if (f) {
                    c(d, f.prototype);
                }

                return d;
            }).apply(null, arguments);
        }

        function c(b, e) {
            return (c = Object.setPrototypeOf || function (b, e) {
                b.__proto__ = e;

                try {
                    if (["number"].indexOf(typeof b.__proto__) < 0) {
                        console.log("_Format__b.__proto__", JSON.stringify(b.__proto__));
                    }
                } catch (e) {
                }

                return b;
            })(b, e);
        }

        function n(b) {
            return function (b) {
                if (Array.isArray(b)) {
                    var e = 0;
                    var f = new Array(b.length);

                    for (; e < b.length; e++) {
                        f[e] = b[e];

                        try {
                            if (["number"].indexOf(typeof f[e]) < 0) {
                                console.log("_Format_2_f[e]", JSON.stringify(f[e]));
                            }
                        } catch (e) {
                        }
                    }

                    return f;
                }
            }(b) || function (b) {
                if (Symbol.iterator in Object(b) || "[object Arguments]" === Object.prototype.toString.call(b)) {
                    return Array.from(b);
                }
            }(b) || function () {
                throw new TypeError("Invalid attempt to spread non-iterable instance");
            }();
        }

        var i = [];
        var r = 0;
        var t = [];
        var o = 0;

        var l = function (b, e) {
            var f = b[e++];
            var a = b[e];
            var d = parseInt("" + f + a, 16);

            if (d >> 7 == 0) {
                return [1, d];
            }

            if (d >> 6 == 2) {
                var c = parseInt("" + b[++e] + b[++e], 16);
                d &= 63;

                try {
                    if (["number"].indexOf(typeof d) < 0) {
                        console.log("_Format_3_d", JSON.stringify(d));
                    }
                } catch (e) {
                }

                return [2, c = (d <<= 8) + c];
            }

            if (d >> 6 == 3) {
                var n = parseInt("" + b[++e] + b[++e], 16);
                var i = parseInt("" + b[++e] + b[++e], 16);
                d &= 63;

                try {
                    if (["number"].indexOf(typeof d) < 0) {
                        console.log("_Format_4_d", JSON.stringify(d));
                    }
                } catch (e) {
                }

                return [3, i = (d <<= 16) + (n <<= 8) + i];
            }
        };

        var u = function (b, e) {
            var f = parseInt("" + b[e] + b[e + 1], 16);
            return f = f > 127 ? -256 + f : f;
        };

        var s = function (b, e) {
            var f = parseInt("" + b[e] + b[e + 1] + b[e + 2] + b[e + 3], 16);
            return f = f > 32767 ? -65536 + f : f;
        };

        var p = function (b, e) {
            var f = parseInt("" + b[e] + b[e + 1] + b[e + 2] + b[e + 3] + b[e + 4] + b[e + 5] + b[e + 6] + b[e + 7], 16);
            return f = f > 2147483647 ? 0 + f : f;
        };

        var y = function (b, e) {
            return parseInt("" + b[e] + b[e + 1], 16);
        };

        var v = function (b, e) {
            return parseInt("" + b[e] + b[e + 1] + b[e + 2] + b[e + 3], 16);
        };

        var g = g || this || window;

        var h = Object.keys || function (b) {
            var e = {};
            var f = 0;

            for (var a in b) {
                e[f++] = a;

                try {
                    if (["number"].indexOf(typeof e.f) < 0) {
                        console.log("_Format_5_e[f++]", JSON.stringify(e[f - 1]));
                    }
                } catch (e) {
                }
            }

            e.length = f;

            try {
                if (["number"].indexOf(typeof e.length) < 0) {
                    console.log("_Format_6_e.length", JSON.stringify(e.length));
                }
            } catch (e) {
            }

            return e;
        };

        var m = (b.length, 0);
        var I = "";
        var C = m;

        for (; C < m + 16; C++) {
            var q = "" + b[C++] + b[C];
            q = parseInt(q, 16);

            try {
                if (["number"].indexOf(typeof q) < 0) {
                    console.log("_Format_7_q", JSON.stringify(q));
                }
            } catch (e) {
            }

            I += String.fromCharCode(q);
        }

        try {
            if (["number"].indexOf(typeof I) < 0) {
                console.log("_Format_8_I", JSON.stringify(I));
            }
        } catch (e) {
        }

        if ("HNOJ@?RC" != I) {
            throw new Error("error magic number " + I);
        }

        m += 16;

        try {
            if (["number"].indexOf(typeof m) < 0) {
                console.log("_Format_9_m", JSON.stringify(m));
            }
        } catch (e) {
        }

        parseInt("" + b[m] + b[m + 1], 16);
        m += 8;

        try {
            if (["number"].indexOf(typeof m) < 0) {
                console.log("_Format_10_m", JSON.stringify(m));
            }
        } catch (e) {
        }

        r = 0;

        try {
            if (["number"].indexOf(typeof r) < 0) {
                console.log("_Format_11_r", JSON.stringify(r));
            }
        } catch (e) {
        }

        var w = 0;

        for (; w < 4; w++) {
            var S = m + 2 * w;
            var R = "" + b[S++] + b[S];
            var x = parseInt(R, 16);
            r += (3 & x) << 2 * w;

            try {
                if (["number"].indexOf(typeof r) < 0) {
                    console.log("_Format_12_r", JSON.stringify(r));
                }
            } catch (e) {
            }
        }

        m += 16;

        try {
            if (["number"].indexOf(typeof m) < 0) {
                console.log("_Format_13_m", JSON.stringify(m));
            }
        } catch (e) {
        }

        m += 8;

        try {
            if (["number"].indexOf(typeof m) < 0) {
                console.log("_Format_14_m", JSON.stringify(m));
            }
        } catch (e) {
        }

        var z = parseInt("" + b[m] + b[m + 1] + b[m + 2] + b[m + 3] + b[m + 4] + b[m + 5] + b[m + 6] + b[m + 7], 16);
        var O = z;
        var E = m += 8;
        var j = v(b, m += z);
        j[1];
        m += 4;

        try {
            if (["number"].indexOf(typeof m) < 0) {
                console.log("_Format_15_m", JSON.stringify(m));
            }
        } catch (e) {
        }

        i = {
            p: [],
            q: []
        };

        try {
            if (["number"].indexOf(typeof i) < 0) {
                console.log("_Format_16_i", JSON.stringify(i));
            }
        } catch (e) {
        }

        var A = 0;

        for (; A < j; A++) {
            var D = l(b, m);
            var T = m += 2 * D[0];
            var $ = i.p.length;
            var P = 0;

            for (; P < D[1]; P++) {
                var U = l(b, T);
                i.p.push(U[1]);
                T += 2 * U[0];

                try {
                    if (["number"].indexOf(typeof T) < 0) {
                        console.log("_Format_17_T", JSON.stringify(T));
                    }
                } catch (e) {
                }
            }

            m = T;

            try {
                if (["number"].indexOf(typeof m) < 0) {
                    console.log("_Format_18_m", JSON.stringify(m));
                }
            } catch (e) {
            }

            i.q.push([$, i.p.length]);
        }

        var _ = {
            5: 1,
            6: 1,
            70: 1,
            22: 1,
            23: 1,
            37: 1,
            73: 1
        };
        var k = {
            72: 1
        };
        var M = {
            74: 1
        };
        var H = {
            11: 1,
            12: 1,
            24: 1,
            26: 1,
            27: 1,
            31: 1
        };
        var J = {
            10: 1
        };
        var N = {
            2: 1,
            29: 1,
            30: 1,
            20: 1
        };
        var B = [];
        var W = [];

        function F(b, e, f) {
            var a = e;

            for (; a < e + f;) {
                var d = y(b, a);
                B[a] = d;

                try {
                    if (["number"].indexOf(typeof B[a]) < 0) {
                        console.log("_Format_19_B[a]", JSON.stringify(B[a]));
                    }
                } catch (e) {
                }

                a += 2;

                try {
                    if (["number"].indexOf(typeof a) < 0) {
                        console.log("_Format_20_a", JSON.stringify(a));
                    }
                } catch (e) {
                }

                if (k[d]) {
                    W[a] = u(b, a);

                    try {
                        if (["number"].indexOf(typeof W[a]) < 0) {
                            console.log("_Format_21_W[a]", JSON.stringify(W[a]));
                        }
                    } catch (e) {
                    }

                    a += 2;

                    try {
                        if (["number"].indexOf(typeof a) < 0) {
                            console.log("_Format_22_a", JSON.stringify(a));
                        }
                    } catch (e) {
                    }
                } else {
                    if (_[d]) {
                        W[a] = s(b, a);

                        try {
                            if (["number"].indexOf(typeof W[a]) < 0) {
                                console.log("_Format_23_W[a]", JSON.stringify(W[a]));
                            }
                        } catch (e) {
                        }

                        a += 4;

                        try {
                            if (["number"].indexOf(typeof a) < 0) {
                                console.log("_Format_24_a", JSON.stringify(a));
                            }
                        } catch (e) {
                        }
                    } else {
                        if (M[d]) {
                            W[a] = p(b, a);

                            try {
                                if (["number"].indexOf(typeof W[a]) < 0) {
                                    console.log("_Format_25_W[a]", JSON.stringify(W[a]));
                                }
                            } catch (e) {
                            }

                            a += 8;

                            try {
                                if (["number"].indexOf(typeof a) < 0) {
                                    console.log("_Format_26_a", JSON.stringify(a));
                                }
                            } catch (e) {
                            }
                        } else {
                            if (H[d]) {
                                W[a] = y(b, a);

                                try {
                                    if (["number"].indexOf(typeof W[a]) < 0) {
                                        console.log("_Format_27_W[a]", JSON.stringify(W[a]));
                                    }
                                } catch (e) {
                                }

                                a += 2;

                                try {
                                    if (["number"].indexOf(typeof a) < 0) {
                                        console.log("_Format_28_a", JSON.stringify(a));
                                    }
                                } catch (e) {
                                }
                            } else {
                                if (J[d]) {
                                    W[a] = v(b, a);

                                    try {
                                        if (["number"].indexOf(typeof W[a]) < 0) {
                                            console.log("_Format_29_W[a]", JSON.stringify(W[a]));
                                        }
                                    } catch (e) {
                                    }

                                    a += 4;

                                    try {
                                        if (["number"].indexOf(typeof a) < 0) {
                                            console.log("_Format_30_a", JSON.stringify(a));
                                        }
                                    } catch (e) {
                                    }
                                } else {
                                    if (N[d]) {
                                        W[a] = v(b, a);

                                        try {
                                            if (["number"].indexOf(typeof W[a]) < 0) {
                                                console.log("_Format_31_W[a]", JSON.stringify(W[a]));
                                            }
                                        } catch (e) {
                                        }

                                        a += 4;

                                        try {
                                            if (["number"].indexOf(typeof a) < 0) {
                                                console.log("_Format_32_a", JSON.stringify(a));
                                            }
                                        } catch (e) {
                                        }
                                    }
                                }
                            }
                        }
                    }
                }
            }
        }

        return K(b, E, O / 2, [], e, f);

        function G(b, e, f, a, c, l, m, I) {
            if (null == l) {
                l = this;

                try {
                    if (["number"].indexOf(typeof l) < 0) {
                        console.log("_Format_33_l", JSON.stringify(l));
                    }
                } catch (e) {
                }
            }

            var C;
            var q;
            var w;
            var S = [];
            var R = 0;

            if (m) {
                C = m;

                try {
                    if (["number"].indexOf(typeof C) < 0) {
                        console.log("_Format_34_C", JSON.stringify(C));
                    }
                } catch (e) {
                }
            }

            var x;
            var z;
            var O = e;
            var E = O + 2 * f;

            if (!I) {
                for (; O < E;) {
                    var j = parseInt("" + b[O] + b[O + 1], 16);
                    O += 2;

                    try {
                        if (["number"].indexOf(typeof O) < 0) {
                            console.log("_Format_35_O", JSON.stringify(O));
                        }
                    } catch (e) {
                    }

                    var A = 3 & (x = 13 * j % 241);

                    if (x >>= 2, A < 1) {
                        A = 3 & x;

                        try {
                            if (["number"].indexOf(typeof A) < 0) {
                                console.log("_Format_36_A", JSON.stringify(A));
                            }
                        } catch (e) {
                        }

                        if (x >>= 2, A > 2) {
                            if ((A = x) > 10) {
                                S[++R] = undefined;

                                try {
                                    if (["number"].indexOf(typeof S.R) < 0) {
                                        console.log("_Format_37_S[++R]", JSON.stringify(S[R - 1]));
                                    }
                                } catch (e) {
                                }
                            } else {
                                if (A > 1) {
                                    C = S[R--];

                                    try {
                                        if (["number"].indexOf(typeof C) < 0) {
                                            console.log("_Format_38_C", JSON.stringify(C));
                                        }
                                    } catch (e) {
                                    }

                                    S[R] = S[R] >= C;

                                    try {
                                        if (["number"].indexOf(typeof S[R]) < 0) {
                                            console.log("_Format_39_S[R]", JSON.stringify(S[R]));
                                        }
                                    } catch (e) {
                                    }
                                } else {
                                    if (A > -1) {
                                        S[++R] = null;

                                        try {
                                            if (["number"].indexOf(typeof S.R) < 0) {
                                                console.log("_Format_40_S[++R]", JSON.stringify(S[R - 1]));
                                            }
                                        } catch (e) {
                                        }
                                    }
                                }
                            }
                        } else {
                            if (A > 1) {
                                if ((A = x) > 11) {
                                    throw S[R--];
                                }

                                if (A > 7) {
                                    C = S[R--];

                                    try {
                                        if (["number"].indexOf(typeof C) < 0) {
                                            console.log("_Format_41_C", JSON.stringify(C));
                                        }
                                    } catch (e) {
                                    }

                                    z = v(b, O);

                                    try {
                                        if (["number"].indexOf(typeof z) < 0) {
                                            console.log("_Format_42_z", JSON.stringify(z));
                                        }
                                    } catch (e) {
                                    }

                                    A = "";

                                    try {
                                        if (["number"].indexOf(typeof A) < 0) {
                                            console.log("_Format_43_A", JSON.stringify(A));
                                        }
                                    } catch (e) {
                                    }

                                    P = i.q[z][0];

                                    try {
                                        if (["number"].indexOf(typeof P) < 0) {
                                            console.log("_Format_44_P", JSON.stringify(P));
                                        }
                                    } catch (e) {
                                    }

                                    for (; P < i.q[z][1]; P++) {
                                        A += String.fromCharCode(r ^ i.p[P]);
                                    }

                                    try {
                                        if (["number"].indexOf(typeof A) < 0) {
                                            console.log("_Format_45_A", JSON.stringify(A));
                                        }
                                    } catch (e) {
                                    }

                                    O += 4;

                                    try {
                                        if (["number"].indexOf(typeof O) < 0) {
                                            console.log("_Format_46_O", JSON.stringify(O));
                                        }
                                    } catch (e) {
                                    }

                                    S[R--][A] = C;
                                } else {
                                    if (A > 5) {
                                        S[R] = h(S[R]);

                                        try {
                                            if (["number"].indexOf(typeof S[R]) < 0) {
                                                console.log("_Format_47_S[R]", JSON.stringify(S[R]));
                                            }
                                        } catch (e) {
                                        }
                                    }
                                }
                            } else {
                                if (A > 0) {
                                    if ((A = x) > 8) {
                                        C = S[R--];

                                        try {
                                            if (["number"].indexOf(typeof C) < 0) {
                                                console.log("_Format_48_C", JSON.stringify(C));
                                            }
                                        } catch (e) {
                                        }

                                        S[R] = typeof C;

                                        try {
                                            if (["number"].indexOf(typeof S[R]) < 0) {
                                                console.log("_Format_49_S[R]", JSON.stringify(S[R]));
                                            }
                                        } catch (e) {
                                        }
                                    } else {
                                        if (A > 6) {
                                            S[R] = --S[R];

                                            try {
                                                if (["number"].indexOf(typeof S[R]) < 0) {
                                                    console.log("_Format_50_S[R]", JSON.stringify(S[R]));
                                                }
                                            } catch (e) {
                                            }
                                        } else {
                                            if (A > 4) {
                                                S[R -= 1] = S[R][S[R + 1]];
                                            } else {
                                                if (A > 2) {
                                                    q = S[R--];

                                                    try {
                                                        if (["number"].indexOf(typeof q) < 0) {
                                                            console.log("_Format_51_q", JSON.stringify(q));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    if ((A = S[R]).x === G) {
                                                        if (A.y >= 1) {
                                                            S[R] = K(b, A.c, A.l, [q], A.z, w, null, 1);

                                                            try {
                                                                if (["number"].indexOf(typeof S[R]) < 0) {
                                                                    console.log("_Format_52_S[R]", JSON.stringify(S[R]));
                                                                }
                                                            } catch (e) {
                                                            }
                                                        } else {
                                                            S[R] = K(b, A.c, A.l, [q], A.z, w, null, 0);

                                                            try {
                                                                if (["number"].indexOf(typeof S[R]) < 0) {
                                                                    console.log("_Format_53_S[R]", JSON.stringify(S[R]));
                                                                }
                                                            } catch (e) {
                                                            }

                                                            A.y++;
                                                        }
                                                    } else {
                                                        S[R] = A(q);

                                                        try {
                                                            if (["number"].indexOf(typeof S[R]) < 0) {
                                                                console.log("_Format_54_S[R]", JSON.stringify(S[R]));
                                                            }
                                                        } catch (e) {
                                                        }
                                                    }
                                                }
                                            }
                                        }
                                    }
                                } else {
                                    if ((A = x) > 14) {
                                        z = s(b, O);

                                        try {
                                            if (["number"].indexOf(typeof z) < 0) {
                                                console.log("_Format_55_z", JSON.stringify(z));
                                            }
                                        } catch (e) {
                                        }

                                        (U = function e() {
                                            var f = arguments;

                                            if (e.y > 0) {
                                                return K(b, e.c, e.l, f, e.z, this, null, 0);
                                            } else {
                                                e.y++;
                                                return K(b, e.c, e.l, f, e.z, this, null, 0);
                                            }
                                        }).c = O + 4;
                                        U.l = z - 2;

                                        try {
                                            if (["number"].indexOf(typeof U.l) < 0) {
                                                console.log("_Format_56_U.l", JSON.stringify(U.l));
                                            }
                                        } catch (e) {
                                        }

                                        U.x = G;

                                        try {
                                            if (["number"].indexOf(typeof U.x) < 0) {
                                                console.log("_Format_57_U.x", JSON.stringify(U.x));
                                            }
                                        } catch (e) {
                                        }

                                        U.y = 0;

                                        try {
                                            if (["number"].indexOf(typeof U.y) < 0) {
                                                console.log("_Format_58_U.y", JSON.stringify(U.y));
                                            }
                                        } catch (e) {
                                        }

                                        U.z = c;

                                        try {
                                            if (["number"].indexOf(typeof U.z) < 0) {
                                                console.log("_Format_59_U.z", JSON.stringify(U.z));
                                            }
                                        } catch (e) {
                                        }

                                        S[R] = U;

                                        try {
                                            if (["number"].indexOf(typeof S[R]) < 0) {
                                                console.log("_Format_60_S[R]", JSON.stringify(S[R]));
                                            }
                                        } catch (e) {
                                        }

                                        O += 2 * z - 2;

                                        try {
                                            if (["number"].indexOf(typeof O) < 0) {
                                                console.log("_Format_61_O", JSON.stringify(O));
                                            }
                                        } catch (e) {
                                        }
                                    } else {
                                        if (A > 12) {
                                            q = S[R--];

                                            try {
                                                if (["number"].indexOf(typeof q) < 0) {
                                                    console.log("_Format_62_q", JSON.stringify(q));
                                                }
                                            } catch (e) {
                                            }

                                            w = S[R--];

                                            try {
                                                if (["number"].indexOf(typeof w) < 0) {
                                                    console.log("_Format_63_w", JSON.stringify(w));
                                                }
                                            } catch (e) {
                                            }

                                            if ((A = S[R--]).x === G) {
                                                if (A.y >= 1) {
                                                    S[++R] = K(b, A.c, A.l, q, A.z, w, null, 1);

                                                    try {
                                                        if (["number"].indexOf(typeof S.R) < 0) {
                                                            console.log("_Format_64_S[++R]", JSON.stringify(S[R - 1]));
                                                        }
                                                    } catch (e) {
                                                    }
                                                } else {
                                                    S[++R] = K(b, A.c, A.l, q, A.z, w, null, 0);

                                                    try {
                                                        if (["number"].indexOf(typeof S.R) < 0) {
                                                            console.log("_Format_65_S[++R]", JSON.stringify(S[R - 1]));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    A.y++;
                                                }
                                            } else {
                                                S[++R] = A.apply(w, q);

                                                try {
                                                    if (["number"].indexOf(typeof S.R) < 0) {
                                                        console.log("_Format_66_S[++R]", JSON.stringify(S[R - 1]));
                                                    }
                                                } catch (e) {
                                                }
                                            }
                                        } else {
                                            if (A > 5) {
                                                C = S[R--];

                                                try {
                                                    if (["number"].indexOf(typeof C) < 0) {
                                                        console.log("_Format_67_C", JSON.stringify(C));
                                                    }
                                                } catch (e) {
                                                }

                                                S[R] = S[R] != C;

                                                try {
                                                    if (["number"].indexOf(typeof S[R]) < 0) {
                                                        console.log("_Format_68_S[R]", JSON.stringify(S[R]));
                                                    }
                                                } catch (e) {
                                                }
                                            } else {
                                                if (A > 3) {
                                                    C = S[R--];

                                                    try {
                                                        if (["number"].indexOf(typeof C) < 0) {
                                                            console.log("_Format_69_C", JSON.stringify(C));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    S[R] = S[R] * C;

                                                    try {
                                                        if (["number"].indexOf(typeof S[R]) < 0) {
                                                            console.log("_Format_70_S[R]", JSON.stringify(S[R]));
                                                        }
                                                    } catch (e) {
                                                    }
                                                } else {
                                                    if (A > -1) {
                                                        return [1, S[R--]];
                                                    }
                                                }
                                            }
                                        }
                                    }
                                }
                            }
                        }
                    } else {
                        if (A < 2) {
                            A = 3 & x;

                            try {
                                if (["number"].indexOf(typeof A) < 0) {
                                    console.log("_Format_71_A", JSON.stringify(A));
                                }
                            } catch (e) {
                            }

                            if (x >>= 2, A < 1) {
                                if ((A = x) > 9) {
                                    ;
                                } else {
                                    if (A > 7) {
                                        C = S[R--];

                                        try {
                                            if (["number"].indexOf(typeof C) < 0) {
                                                console.log("_Format_72_C", JSON.stringify(C));
                                            }
                                        } catch (e) {
                                        }

                                        S[R] = S[R] & C;

                                        try {
                                            if (["number"].indexOf(typeof S[R]) < 0) {
                                                console.log("_Format_73_S[R]", JSON.stringify(S[R]));
                                            }
                                        } catch (e) {
                                        }
                                    } else {
                                        if (A > 5) {
                                            z = y(b, O);

                                            try {
                                                if (["number"].indexOf(typeof z) < 0) {
                                                    console.log("_Format_74_z", JSON.stringify(z));
                                                }
                                            } catch (e) {
                                            }

                                            O += 2;

                                            try {
                                                if (["number"].indexOf(typeof O) < 0) {
                                                    console.log("_Format_75_O", JSON.stringify(O));
                                                }
                                            } catch (e) {
                                            }

                                            S[R -= z] = 0 === z ? new S[R]() : d(S[R], n(S.slice(R + 1, R + z + 1)));
                                        } else {
                                            if (A > 3) {
                                                z = s(b, O);

                                                try {
                                                    if (["number"].indexOf(typeof z) < 0) {
                                                        console.log("_Format_76_z", JSON.stringify(z));
                                                    }
                                                } catch (e) {
                                                }

                                                try {
                                                    if (t[o][2] = 1, 1 == (C = G(b, O + 4, z - 3, [], c, l, null, 0))[0]) {
                                                        return C;
                                                    }
                                                } catch (m) {
                                                    if (t[o] && t[o][1] && 1 == (C = G(b, t[o][1][0], t[o][1][1], [], c, l, m, 0))[0]) {
                                                        return C;
                                                    }
                                                } finally {
                                                    if (t[o] && t[o][0] && 1 == (C = G(b, t[o][0][0], t[o][0][1], [], c, l, null, 0))[0]) {
                                                        return C;
                                                    }

                                                    t[o] = 0;

                                                    try {
                                                        if (["number"].indexOf(typeof t[o]) < 0) {
                                                            console.log("_Format_77_t[o]", JSON.stringify(t[o]));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    o--;
                                                }

                                                O += 2 * z - 2;

                                                try {
                                                    if (["number"].indexOf(typeof O) < 0) {
                                                        console.log("_Format_78_O", JSON.stringify(O));
                                                    }
                                                } catch (e) {
                                                }
                                            }
                                        }
                                    }
                                }
                            } else {
                                if (A < 2) {
                                    if ((A = x) > 12) {
                                        S[++R] = u(b, O);

                                        try {
                                            if (["number"].indexOf(typeof S.R) < 0) {
                                                console.log("_Format_79_S[++R]", JSON.stringify(S[R - 1]));
                                            }
                                        } catch (e) {
                                        }

                                        O += 2;

                                        try {
                                            if (["number"].indexOf(typeof O) < 0) {
                                                console.log("_Format_80_O", JSON.stringify(O));
                                            }
                                        } catch (e) {
                                        }
                                    } else {
                                        if (A > 10) {
                                            C = S[R--];

                                            try {
                                                if (["number"].indexOf(typeof C) < 0) {
                                                    console.log("_Format_81_C", JSON.stringify(C));
                                                }
                                            } catch (e) {
                                            }

                                            S[R] = S[R] << C;

                                            try {
                                                if (["number"].indexOf(typeof S[R]) < 0) {
                                                    console.log("_Format_82_S[R]", JSON.stringify(S[R]));
                                                }
                                            } catch (e) {
                                            }
                                        } else {
                                            if (A > 8) {
                                                z = v(b, O);

                                                try {
                                                    if (["number"].indexOf(typeof z) < 0) {
                                                        console.log("_Format_83_z", JSON.stringify(z));
                                                    }
                                                } catch (e) {
                                                }

                                                A = "";

                                                try {
                                                    if (["number"].indexOf(typeof A) < 0) {
                                                        console.log("_Format_84_A", JSON.stringify(A));
                                                    }
                                                } catch (e) {
                                                }

                                                P = i.q[z][0];

                                                try {
                                                    if (["number"].indexOf(typeof P) < 0) {
                                                        console.log("_Format_85_P", JSON.stringify(P));
                                                    }
                                                } catch (e) {
                                                }

                                                for (; P < i.q[z][1]; P++) {
                                                    A += String.fromCharCode(r ^ i.p[P]);
                                                }

                                                try {
                                                    if (["number"].indexOf(typeof A) < 0) {
                                                        console.log("_Format_86_A", JSON.stringify(A));
                                                    }
                                                } catch (e) {
                                                }

                                                O += 4;

                                                try {
                                                    if (["number"].indexOf(typeof O) < 0) {
                                                        console.log("_Format_87_O", JSON.stringify(O));
                                                    }
                                                } catch (e) {
                                                }

                                                S[R] = S[R][A];

                                                try {
                                                    if (["number"].indexOf(typeof S[R]) < 0) {
                                                        console.log("_Format_88_S[R]", JSON.stringify(S[R]));
                                                    }
                                                } catch (e) {
                                                }
                                            } else {
                                                if (A > 6) {
                                                    q = S[R--];

                                                    try {
                                                        if (["number"].indexOf(typeof q) < 0) {
                                                            console.log("_Format_89_q", JSON.stringify(q));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    C = delete S[R--][q];

                                                    try {
                                                        if (["number"].indexOf(typeof C) < 0) {
                                                            console.log("_Format_90_C", JSON.stringify(C));
                                                        }
                                                    } catch (e) {
                                                    }
                                                }
                                            }
                                        }
                                    }
                                } else {
                                    if (A < 3) {
                                        if ((A = x) < 2) {
                                            S[++R] = C;

                                            try {
                                                if (["number"].indexOf(typeof S.R) < 0) {
                                                    console.log("_Format_91_S[++R]", JSON.stringify(S[R - 1]));
                                                }
                                            } catch (e) {
                                            }
                                        } else {
                                            if (A < 4) {
                                                C = S[R--];

                                                try {
                                                    if (["number"].indexOf(typeof C) < 0) {
                                                        console.log("_Format_92_C", JSON.stringify(C));
                                                    }
                                                } catch (e) {
                                                }

                                                S[R] = S[R] <= C;

                                                try {
                                                    if (["number"].indexOf(typeof S[R]) < 0) {
                                                        console.log("_Format_93_S[R]", JSON.stringify(S[R]));
                                                    }
                                                } catch (e) {
                                                }
                                            } else {
                                                if (A < 11) {
                                                    C = S[R -= 2][S[R + 1]] = S[R + 2];

                                                    try {
                                                        if (["number"].indexOf(typeof C) < 0) {
                                                            console.log("_Format_94_C", JSON.stringify(C));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    R--;
                                                } else {
                                                    if (A < 13) {
                                                        C = S[R];

                                                        try {
                                                            if (["number"].indexOf(typeof C) < 0) {
                                                                console.log("_Format_95_C", JSON.stringify(C));
                                                            }
                                                        } catch (e) {
                                                        }

                                                        S[++R] = C;

                                                        try {
                                                            if (["number"].indexOf(typeof S.R) < 0) {
                                                                console.log("_Format_96_S[++R]", JSON.stringify(S[R - 1]));
                                                            }
                                                        } catch (e) {
                                                        }
                                                    }
                                                }
                                            }
                                        }
                                    } else {
                                        if ((A = x) > 12) {
                                            S[++R] = l;

                                            try {
                                                if (["number"].indexOf(typeof S.R) < 0) {
                                                    console.log("_Format_97_S[++R]", JSON.stringify(S[R - 1]));
                                                }
                                            } catch (e) {
                                            }
                                        } else {
                                            if (A > 5) {
                                                C = S[R--];

                                                try {
                                                    if (["number"].indexOf(typeof C) < 0) {
                                                        console.log("_Format_98_C", JSON.stringify(C));
                                                    }
                                                } catch (e) {
                                                }

                                                S[R] = S[R] !== C;

                                                try {
                                                    if (["number"].indexOf(typeof S[R]) < 0) {
                                                        console.log("_Format_99_S[R]", JSON.stringify(S[R]));
                                                    }
                                                } catch (e) {
                                                }
                                            } else {
                                                if (A > 3) {
                                                    C = S[R--];

                                                    try {
                                                        if (["number"].indexOf(typeof C) < 0) {
                                                            console.log("_Format_100_C", JSON.stringify(C));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    S[R] = S[R] / C;

                                                    try {
                                                        if (["number"].indexOf(typeof S[R]) < 0) {
                                                            console.log("_Format_101_S[R]", JSON.stringify(S[R]));
                                                        }
                                                    } catch (e) {
                                                    }
                                                } else {
                                                    if (A > 1) {
                                                        if ((z = s(b, O)) < 0) {
                                                            I = 1;

                                                            try {
                                                                if (["number"].indexOf(typeof I) < 0) {
                                                                    console.log("_Format_102_I", JSON.stringify(I));
                                                                }
                                                            } catch (e) {
                                                            }

                                                            F(b, e, 2 * f);
                                                            O += 2 * z - 2;

                                                            try {
                                                                if (["number"].indexOf(typeof O) < 0) {
                                                                    console.log("_Format_103_O", JSON.stringify(O));
                                                                }
                                                            } catch (e) {
                                                            }

                                                            break;
                                                        }

                                                        O += 2 * z - 2;

                                                        try {
                                                            if (["number"].indexOf(typeof O) < 0) {
                                                                console.log("_Format_104_O", JSON.stringify(O));
                                                            }
                                                        } catch (e) {
                                                        }
                                                    } else {
                                                        if (A > -1) {
                                                            S[R] = !S[R];

                                                            try {
                                                                if (["number"].indexOf(typeof S[R]) < 0) {
                                                                    console.log("_Format_105_S[R]", JSON.stringify(S[R]));
                                                                }
                                                            } catch (e) {
                                                            }
                                                        }
                                                    }
                                                }
                                            }
                                        }
                                    }
                                }
                            }
                        } else {
                            if (A < 3) {
                                A = 3 & x;

                                try {
                                    if (["number"].indexOf(typeof A) < 0) {
                                        console.log("_Format_106_A", JSON.stringify(A));
                                    }
                                } catch (e) {
                                }

                                if (x >>= 2, A > 2) {
                                    if ((A = x) > 7) {
                                        C = S[R--];

                                        try {
                                            if (["number"].indexOf(typeof C) < 0) {
                                                console.log("_Format_107_C", JSON.stringify(C));
                                            }
                                        } catch (e) {
                                        }

                                        S[R] = S[R] | C;

                                        try {
                                            if (["number"].indexOf(typeof S[R]) < 0) {
                                                console.log("_Format_108_S[R]", JSON.stringify(S[R]));
                                            }
                                        } catch (e) {
                                        }
                                    } else {
                                        if (A > 5) {
                                            z = y(b, O);

                                            try {
                                                if (["number"].indexOf(typeof z) < 0) {
                                                    console.log("_Format_109_z", JSON.stringify(z));
                                                }
                                            } catch (e) {
                                            }

                                            O += 2;

                                            try {
                                                if (["number"].indexOf(typeof O) < 0) {
                                                    console.log("_Format_110_O", JSON.stringify(O));
                                                }
                                            } catch (e) {
                                            }

                                            S[++R] = c["$" + z];

                                            try {
                                                if (["number"].indexOf(typeof S.R) < 0) {
                                                    console.log("_Format_111_S[++R]", JSON.stringify(S[R - 1]));
                                                }
                                            } catch (e) {
                                            }
                                        } else {
                                            if (A > 3) {
                                                z = s(b, O);

                                                try {
                                                    if (["number"].indexOf(typeof z) < 0) {
                                                        console.log("_Format_112_z", JSON.stringify(z));
                                                    }
                                                } catch (e) {
                                                }

                                                if (t[o][0] && !t[o][2]) {
                                                    t[o][1] = [O + 4, z - 3];
                                                } else {
                                                    t[o++] = [0, [O + 4, z - 3], 0];

                                                    try {
                                                        if (["number"].indexOf(typeof t.o) < 0) {
                                                            console.log("_Format_113_t[o++]", JSON.stringify(t[o - 1]));
                                                        }
                                                    } catch (e) {
                                                    }
                                                }

                                                O += 2 * z - 2;

                                                try {
                                                    if (["number"].indexOf(typeof O) < 0) {
                                                        console.log("_Format_114_O", JSON.stringify(O));
                                                    }
                                                } catch (e) {
                                                }
                                            }
                                        }
                                    }
                                } else {
                                    if (A > 1) {
                                        if ((A = x) < 2) {
                                            z = v(b, O);

                                            try {
                                                if (["number"].indexOf(typeof z) < 0) {
                                                    console.log("_Format_115_z", JSON.stringify(z));
                                                }
                                            } catch (e) {
                                            }

                                            C = "";

                                            try {
                                                if (["number"].indexOf(typeof C) < 0) {
                                                    console.log("_Format_116_C", JSON.stringify(C));
                                                }
                                            } catch (e) {
                                            }

                                            P = i.q[z][0];

                                            try {
                                                if (["number"].indexOf(typeof P) < 0) {
                                                    console.log("_Format_117_P", JSON.stringify(P));
                                                }
                                            } catch (e) {
                                            }

                                            for (; P < i.q[z][1]; P++) {
                                                C += String.fromCharCode(r ^ i.p[P]);
                                            }

                                            try {
                                                if (["number"].indexOf(typeof C) < 0) {
                                                    console.log("_Format_118_C", JSON.stringify(C));
                                                }
                                            } catch (e) {
                                            }

                                            S[++R] = C;

                                            try {
                                                if (["number"].indexOf(typeof S.R) < 0) {
                                                    console.log("_Format_119_S[++R]", JSON.stringify(S[R - 1]));
                                                }
                                            } catch (e) {
                                            }

                                            O += 4;

                                            try {
                                                if (["number"].indexOf(typeof O) < 0) {
                                                    console.log("_Format_120_O", JSON.stringify(O));
                                                }
                                            } catch (e) {
                                            }
                                        } else {
                                            if (A < 4) {
                                                if (S[R--]) {
                                                    O += 4;

                                                    try {
                                                        if (["number"].indexOf(typeof O) < 0) {
                                                            console.log("_Format_121_O", JSON.stringify(O));
                                                        }
                                                    } catch (e) {
                                                    }
                                                } else {
                                                    if ((z = s(b, O)) < 0) {
                                                        I = 1;

                                                        try {
                                                            if (["number"].indexOf(typeof I) < 0) {
                                                                console.log("_Format_122_I", JSON.stringify(I));
                                                            }
                                                        } catch (e) {
                                                        }

                                                        F(b, e, 2 * f);
                                                        O += 2 * z - 2;

                                                        try {
                                                            if (["number"].indexOf(typeof O) < 0) {
                                                                console.log("_Format_123_O", JSON.stringify(O));
                                                            }
                                                        } catch (e) {
                                                        }

                                                        break;
                                                    }

                                                    O += 2 * z - 2;

                                                    try {
                                                        if (["number"].indexOf(typeof O) < 0) {
                                                            console.log("_Format_124_O", JSON.stringify(O));
                                                        }
                                                    } catch (e) {
                                                    }
                                                }
                                            } else {
                                                if (A < 6) {
                                                    C = S[R--];

                                                    try {
                                                        if (["number"].indexOf(typeof C) < 0) {
                                                            console.log("_Format_125_C", JSON.stringify(C));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    S[R] = S[R] % C;

                                                    try {
                                                        if (["number"].indexOf(typeof S[R]) < 0) {
                                                            console.log("_Format_126_S[R]", JSON.stringify(S[R]));
                                                        }
                                                    } catch (e) {
                                                    }
                                                } else {
                                                    if (A < 8) {
                                                        C = S[R--];

                                                        try {
                                                            if (["number"].indexOf(typeof C) < 0) {
                                                                console.log("_Format_127_C", JSON.stringify(C));
                                                            }
                                                        } catch (e) {
                                                        }

                                                        S[R] = S[R] instanceof C;

                                                        try {
                                                            if (["number"].indexOf(typeof S[R]) < 0) {
                                                                console.log("_Format_128_S[R]", JSON.stringify(S[R]));
                                                            }
                                                        } catch (e) {
                                                        }
                                                    } else {
                                                        if (A < 15) {
                                                            S[++R] = !1;

                                                            try {
                                                                if (["number"].indexOf(typeof S.R) < 0) {
                                                                    console.log("_Format_129_S[++R]", JSON.stringify(S[R - 1]));
                                                                }
                                                            } catch (e) {
                                                            }
                                                        }
                                                    }
                                                }
                                            }
                                        }
                                    } else {
                                        if (A > 0) {
                                            if ((A = x) < 1) {
                                                S[++R] = g;

                                                try {
                                                    if (["number"].indexOf(typeof S.R) < 0) {
                                                        console.log("_Format_130_S[++R]", JSON.stringify(S[R - 1]));
                                                    }
                                                } catch (e) {
                                                }
                                            } else {
                                                if (A < 3) {
                                                    C = S[R--];

                                                    try {
                                                        if (["number"].indexOf(typeof C) < 0) {
                                                            console.log("_Format_131_C", JSON.stringify(C));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    S[R] = S[R] + C;

                                                    try {
                                                        if (["number"].indexOf(typeof S[R]) < 0) {
                                                            console.log("_Format_132_S[R]", JSON.stringify(S[R]));
                                                        }
                                                    } catch (e) {
                                                    }
                                                } else {
                                                    if (A < 5) {
                                                        C = S[R--];

                                                        try {
                                                            if (["number"].indexOf(typeof C) < 0) {
                                                                console.log("_Format_133_C", JSON.stringify(C));
                                                            }
                                                        } catch (e) {
                                                        }

                                                        S[R] = S[R] == C;

                                                        try {
                                                            if (["number"].indexOf(typeof S[R]) < 0) {
                                                                console.log("_Format_134_S[R]", JSON.stringify(S[R]));
                                                            }
                                                        } catch (e) {
                                                        }
                                                    } else {
                                                        if (A < 14) {
                                                            C = S[R - 1];

                                                            try {
                                                                if (["number"].indexOf(typeof C) < 0) {
                                                                    console.log("_Format_135_C", JSON.stringify(C));
                                                                }
                                                            } catch (e) {
                                                            }

                                                            q = S[R];

                                                            try {
                                                                if (["number"].indexOf(typeof q) < 0) {
                                                                    console.log("_Format_136_q", JSON.stringify(q));
                                                                }
                                                            } catch (e) {
                                                            }

                                                            S[++R] = C;

                                                            try {
                                                                if (["number"].indexOf(typeof S.R) < 0) {
                                                                    console.log("_Format_137_S[++R]", JSON.stringify(S[R - 1]));
                                                                }
                                                            } catch (e) {
                                                            }

                                                            S[++R] = q;

                                                            try {
                                                                if (["number"].indexOf(typeof S.R) < 0) {
                                                                    console.log("_Format_138_S[++R]", JSON.stringify(S[R - 1]));
                                                                }
                                                            } catch (e) {
                                                            }
                                                        }
                                                    }
                                                }
                                            }
                                        } else {
                                            if ((A = x) < 2) {
                                                C = S[R--];

                                                try {
                                                    if (["number"].indexOf(typeof C) < 0) {
                                                        console.log("_Format_139_C", JSON.stringify(C));
                                                    }
                                                } catch (e) {
                                                }

                                                S[R] = S[R] > C;

                                                try {
                                                    if (["number"].indexOf(typeof S[R]) < 0) {
                                                        console.log("_Format_140_S[R]", JSON.stringify(S[R]));
                                                    }
                                                } catch (e) {
                                                }
                                            } else {
                                                if (A < 9) {
                                                    z = v(b, O);

                                                    try {
                                                        if (["number"].indexOf(typeof z) < 0) {
                                                            console.log("_Format_141_z", JSON.stringify(z));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    O += 4;

                                                    try {
                                                        if (["number"].indexOf(typeof O) < 0) {
                                                            console.log("_Format_142_O", JSON.stringify(O));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    q = R + 1;

                                                    try {
                                                        if (["number"].indexOf(typeof q) < 0) {
                                                            console.log("_Format_143_q", JSON.stringify(q));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    S[R -= z - 1] = z ? S.slice(R, q) : [];
                                                } else {
                                                    if (A < 11) {
                                                        z = y(b, O);

                                                        try {
                                                            if (["number"].indexOf(typeof z) < 0) {
                                                                console.log("_Format_144_z", JSON.stringify(z));
                                                            }
                                                        } catch (e) {
                                                        }

                                                        O += 2;

                                                        try {
                                                            if (["number"].indexOf(typeof O) < 0) {
                                                                console.log("_Format_145_O", JSON.stringify(O));
                                                            }
                                                        } catch (e) {
                                                        }

                                                        C = S[R--];

                                                        try {
                                                            if (["number"].indexOf(typeof C) < 0) {
                                                                console.log("_Format_146_C", JSON.stringify(C));
                                                            }
                                                        } catch (e) {
                                                        }

                                                        c[z] = C;

                                                        try {
                                                            if (["number"].indexOf(typeof c[z]) < 0) {
                                                                console.log("_Format_147_c[z]", JSON.stringify(c[z]));
                                                            }
                                                        } catch (e) {
                                                        }
                                                    } else {
                                                        if (A < 13) {
                                                            C = S[R--];

                                                            try {
                                                                if (["number"].indexOf(typeof C) < 0) {
                                                                    console.log("_Format_148_C", JSON.stringify(C));
                                                                }
                                                            } catch (e) {
                                                            }

                                                            S[R] = S[R] >> C;

                                                            try {
                                                                if (["number"].indexOf(typeof S[R]) < 0) {
                                                                    console.log("_Format_149_S[R]", JSON.stringify(S[R]));
                                                                }
                                                            } catch (e) {
                                                            }
                                                        } else {
                                                            if (A < 15) {
                                                                S[++R] = s(b, O);

                                                                try {
                                                                    if (["number"].indexOf(typeof S.R) < 0) {
                                                                        console.log("_Format_150_S[++R]", JSON.stringify(S[R - 1]));
                                                                    }
                                                                } catch (e) {
                                                                }

                                                                O += 4;

                                                                try {
                                                                    if (["number"].indexOf(typeof O) < 0) {
                                                                        console.log("_Format_151_O", JSON.stringify(O));
                                                                    }
                                                                } catch (e) {
                                                                }
                                                            }
                                                        }
                                                    }
                                                }
                                            }
                                        }
                                    }
                                }
                            } else {
                                A = 3 & x;

                                try {
                                    if (["number"].indexOf(typeof A) < 0) {
                                        console.log("_Format_152_A", JSON.stringify(A));
                                    }
                                } catch (e) {
                                }

                                if (x >>= 2, A > 2) {
                                    if ((A = x) > 13) {
                                        S[++R] = p(b, O);

                                        try {
                                            if (["number"].indexOf(typeof S.R) < 0) {
                                                console.log("_Format_153_S[++R]", JSON.stringify(S[R - 1]));
                                            }
                                        } catch (e) {
                                        }

                                        O += 8;

                                        try {
                                            if (["number"].indexOf(typeof O) < 0) {
                                                console.log("_Format_154_O", JSON.stringify(O));
                                            }
                                        } catch (e) {
                                        }
                                    } else {
                                        if (A > 11) {
                                            C = S[R--];

                                            try {
                                                if (["number"].indexOf(typeof C) < 0) {
                                                    console.log("_Format_155_C", JSON.stringify(C));
                                                }
                                            } catch (e) {
                                            }

                                            S[R] = S[R] >>> C;

                                            try {
                                                if (["number"].indexOf(typeof S[R]) < 0) {
                                                    console.log("_Format_156_S[R]", JSON.stringify(S[R]));
                                                }
                                            } catch (e) {
                                            }
                                        } else {
                                            if (A > 9) {
                                                S[++R] = !0;

                                                try {
                                                    if (["number"].indexOf(typeof S.R) < 0) {
                                                        console.log("_Format_157_S[++R]", JSON.stringify(S[R - 1]));
                                                    }
                                                } catch (e) {
                                                }
                                            } else {
                                                if (A > 7) {
                                                    z = y(b, O);

                                                    try {
                                                        if (["number"].indexOf(typeof z) < 0) {
                                                            console.log("_Format_158_z", JSON.stringify(z));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    O += 2;

                                                    try {
                                                        if (["number"].indexOf(typeof O) < 0) {
                                                            console.log("_Format_159_O", JSON.stringify(O));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    S[R] = S[R][z];

                                                    try {
                                                        if (["number"].indexOf(typeof S[R]) < 0) {
                                                            console.log("_Format_160_S[R]", JSON.stringify(S[R]));
                                                        }
                                                    } catch (e) {
                                                    }
                                                } else {
                                                    if (A > 0) {
                                                        C = S[R--];

                                                        try {
                                                            if (["number"].indexOf(typeof C) < 0) {
                                                                console.log("_Format_161_C", JSON.stringify(C));
                                                            }
                                                        } catch (e) {
                                                        }

                                                        S[R] = S[R] < C;

                                                        try {
                                                            if (["number"].indexOf(typeof S[R]) < 0) {
                                                                console.log("_Format_162_S[R]", JSON.stringify(S[R]));
                                                            }
                                                        } catch (e) {
                                                        }
                                                    }
                                                }
                                            }
                                        }
                                    }
                                } else {
                                    if (A > 1) {
                                        if ((A = x) > 10) {
                                            z = s(b, O);

                                            try {
                                                if (["number"].indexOf(typeof z) < 0) {
                                                    console.log("_Format_163_z", JSON.stringify(z));
                                                }
                                            } catch (e) {
                                            }

                                            t[++o] = [[O + 4, z - 3], 0, 0];

                                            try {
                                                if (["number"].indexOf(typeof t.o) < 0) {
                                                    console.log("_Format_164_t[++o]", JSON.stringify(t[o - 1]));
                                                }
                                            } catch (e) {
                                            }

                                            O += 2 * z - 2;

                                            try {
                                                if (["number"].indexOf(typeof O) < 0) {
                                                    console.log("_Format_165_O", JSON.stringify(O));
                                                }
                                            } catch (e) {
                                            }
                                        } else {
                                            if (A > 8) {
                                                C = S[R--];

                                                try {
                                                    if (["number"].indexOf(typeof C) < 0) {
                                                        console.log("_Format_166_C", JSON.stringify(C));
                                                    }
                                                } catch (e) {
                                                }

                                                S[R] = S[R] ^ C;

                                                try {
                                                    if (["number"].indexOf(typeof S[R]) < 0) {
                                                        console.log("_Format_167_S[R]", JSON.stringify(S[R]));
                                                    }
                                                } catch (e) {
                                                }
                                            } else {
                                                if (A > 6) {
                                                    C = S[R--];

                                                    try {
                                                        if (["number"].indexOf(typeof C) < 0) {
                                                            console.log("_Format_168_C", JSON.stringify(C));
                                                        }
                                                    } catch (e) {
                                                    }
                                                }
                                            }
                                        }
                                    } else {
                                        if (A > 0) {
                                            if ((A = x) < 3) {
                                                var D = 0;
                                                var T = S[R].length;
                                                var $ = S[R];

                                                S[++R] = function () {
                                                    var b = D < T;

                                                    if (b) {
                                                        var e = $[D++];
                                                        S[++R] = e;
                                                    }

                                                    S[++R] = b;
                                                };

                                                try {
                                                    if (["number"].indexOf(typeof S.R) < 0) {
                                                        console.log("_Format_169_S[++R]", JSON.stringify(S[R - 1]));
                                                    }
                                                } catch (e) {
                                                }
                                            } else {
                                                if (A < 5) {
                                                    z = y(b, O);

                                                    try {
                                                        if (["number"].indexOf(typeof z) < 0) {
                                                            console.log("_Format_170_z", JSON.stringify(z));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    O += 2;

                                                    try {
                                                        if (["number"].indexOf(typeof O) < 0) {
                                                            console.log("_Format_171_O", JSON.stringify(O));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    C = c[z];

                                                    try {
                                                        if (["number"].indexOf(typeof C) < 0) {
                                                            console.log("_Format_172_C", JSON.stringify(C));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    S[++R] = C;

                                                    try {
                                                        if (["number"].indexOf(typeof S.R) < 0) {
                                                            console.log("_Format_173_S[++R]", JSON.stringify(S[R - 1]));
                                                        }
                                                    } catch (e) {
                                                    }
                                                } else {
                                                    if (A < 7) {
                                                        S[R] = ++S[R];

                                                        try {
                                                            if (["number"].indexOf(typeof S[R]) < 0) {
                                                                console.log("_Format_174_S[R]", JSON.stringify(S[R]));
                                                            }
                                                        } catch (e) {
                                                        }
                                                    } else {
                                                        if (A < 9) {
                                                            C = S[R--];

                                                            try {
                                                                if (["number"].indexOf(typeof C) < 0) {
                                                                    console.log("_Format_175_C", JSON.stringify(C));
                                                                }
                                                            } catch (e) {
                                                            }

                                                            S[R] = S[R] in C;

                                                            try {
                                                                if (["number"].indexOf(typeof S[R]) < 0) {
                                                                    console.log("_Format_176_S[R]", JSON.stringify(S[R]));
                                                                }
                                                            } catch (e) {
                                                            }
                                                        }
                                                    }
                                                }
                                            }
                                        } else {
                                            if ((A = x) > 13) {
                                                C = S[R];

                                                try {
                                                    if (["number"].indexOf(typeof C) < 0) {
                                                        console.log("_Format_177_C", JSON.stringify(C));
                                                    }
                                                } catch (e) {
                                                }

                                                S[R] = S[R - 1];

                                                try {
                                                    if (["number"].indexOf(typeof S[R]) < 0) {
                                                        console.log("_Format_178_S[R]", JSON.stringify(S[R]));
                                                    }
                                                } catch (e) {
                                                }

                                                S[R - 1] = C;
                                            } else {
                                                if (A > 4) {
                                                    C = S[R--];

                                                    try {
                                                        if (["number"].indexOf(typeof C) < 0) {
                                                            console.log("_Format_179_C", JSON.stringify(C));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    S[R] = S[R] === C;

                                                    try {
                                                        if (["number"].indexOf(typeof S[R]) < 0) {
                                                            console.log("_Format_180_S[R]", JSON.stringify(S[R]));
                                                        }
                                                    } catch (e) {
                                                    }
                                                } else {
                                                    if (A > 2) {
                                                        C = S[R--];

                                                        try {
                                                            if (["number"].indexOf(typeof C) < 0) {
                                                                console.log("_Format_181_C", JSON.stringify(C));
                                                            }
                                                        } catch (e) {
                                                        }

                                                        S[R] = S[R] - C;

                                                        try {
                                                            if (["number"].indexOf(typeof S[R]) < 0) {
                                                                console.log("_Format_182_S[R]", JSON.stringify(S[R]));
                                                            }
                                                        } catch (e) {
                                                        }
                                                    } else {
                                                        if (A > 0) {
                                                            z = v(b, O);

                                                            try {
                                                                if (["number"].indexOf(typeof z) < 0) {
                                                                    console.log("_Format_183_z", JSON.stringify(z));
                                                                }
                                                            } catch (e) {
                                                            }

                                                            A = "";

                                                            try {
                                                                if (["number"].indexOf(typeof A) < 0) {
                                                                    console.log("_Format_184_A", JSON.stringify(A));
                                                                }
                                                            } catch (e) {
                                                            }

                                                            P = i.q[z][0];

                                                            try {
                                                                if (["number"].indexOf(typeof P) < 0) {
                                                                    console.log("_Format_185_P", JSON.stringify(P));
                                                                }
                                                            } catch (e) {
                                                            }

                                                            for (; P < i.q[z][1]; P++) {
                                                                A += String.fromCharCode(r ^ i.p[P]);
                                                            }

                                                            try {
                                                                if (["number"].indexOf(typeof A) < 0) {
                                                                    console.log("_Format_186_A", JSON.stringify(A));
                                                                }
                                                            } catch (e) {
                                                            }

                                                            A = +A;

                                                            try {
                                                                if (["number"].indexOf(typeof A) < 0) {
                                                                    console.log("_Format_187_A", JSON.stringify(A));
                                                                }
                                                            } catch (e) {
                                                            }

                                                            O += 4;

                                                            try {
                                                                if (["number"].indexOf(typeof O) < 0) {
                                                                    console.log("_Format_188_O", JSON.stringify(O));
                                                                }
                                                            } catch (e) {
                                                            }

                                                            S[++R] = A;

                                                            try {
                                                                if (["number"].indexOf(typeof S.R) < 0) {
                                                                    console.log("_Format_189_S[++R]", JSON.stringify(S[R - 1]));
                                                                }
                                                            } catch (e) {
                                                            }
                                                        }
                                                    }
                                                }
                                            }
                                        }
                                    }
                                }
                            }
                        }
                    }
                }
            }

            if (I) {
                for (; O < E;) {
                    j = B[O];

                    try {
                        if (["number"].indexOf(typeof j) < 0) {
                            console.log("_Format_190_j", JSON.stringify(j));
                        }
                    } catch (e) {
                    }

                    O += 2;

                    try {
                        if (["number"].indexOf(typeof O) < 0) {
                            console.log("_Format_191_O", JSON.stringify(O));
                        }
                    } catch (e) {
                    }

                    A = 3 & (x = 13 * j % 241);

                    try {
                        if (["number"].indexOf(typeof A) < 0) {
                            console.log("_Format_192_A", JSON.stringify(A));
                        }
                    } catch (e) {
                    }

                    if (x >>= 2, A < 1) {
                        var U;
                        A = 3 & x;

                        try {
                            if (["number"].indexOf(typeof A) < 0) {
                                console.log("_Format_193_A", JSON.stringify(A));
                            }
                        } catch (e) {
                        }

                        if (x >>= 2, A < 1) {
                            if ((A = x) > 14) {
                                z = W[O];

                                try {
                                    if (["number"].indexOf(typeof z) < 0) {
                                        console.log("_Format_194_z", JSON.stringify(z));
                                    }
                                } catch (e) {
                                }

                                (U = function e() {
                                    var f = arguments;

                                    if (e.y > 0) {
                                        return K(b, e.c, e.l, f, e.z, this, null, 0);
                                    } else {
                                        e.y++;
                                        return K(b, e.c, e.l, f, e.z, this, null, 0);
                                    }
                                }).c = O + 4;
                                U.l = z - 2;

                                try {
                                    if (["number"].indexOf(typeof U.l) < 0) {
                                        console.log("_Format_195_U.l", JSON.stringify(U.l));
                                    }
                                } catch (e) {
                                }

                                U.x = G;

                                try {
                                    if (["number"].indexOf(typeof U.x) < 0) {
                                        console.log("_Format_196_U.x", JSON.stringify(U.x));
                                    }
                                } catch (e) {
                                }

                                U.y = 0;

                                try {
                                    if (["number"].indexOf(typeof U.y) < 0) {
                                        console.log("_Format_197_U.y", JSON.stringify(U.y));
                                    }
                                } catch (e) {
                                }

                                U.z = c;

                                try {
                                    if (["number"].indexOf(typeof U.z) < 0) {
                                        console.log("_Format_198_U.z", JSON.stringify(U.z));
                                    }
                                } catch (e) {
                                }

                                S[R] = U;

                                try {
                                    if (["number"].indexOf(typeof S[R]) < 0) {
                                        console.log("_Format_199_S[R]", JSON.stringify(S[R]));
                                    }
                                } catch (e) {
                                }

                                O += 2 * z - 2;

                                try {
                                    if (["number"].indexOf(typeof O) < 0) {
                                        console.log("_Format_200_O", JSON.stringify(O));
                                    }
                                } catch (e) {
                                }
                            } else {
                                if (A > 12) {
                                    q = S[R--];

                                    try {
                                        if (["number"].indexOf(typeof q) < 0) {
                                            console.log("_Format_201_q", JSON.stringify(q));
                                        }
                                    } catch (e) {
                                    }

                                    w = S[R--];

                                    try {
                                        if (["number"].indexOf(typeof w) < 0) {
                                            console.log("_Format_202_w", JSON.stringify(w));
                                        }
                                    } catch (e) {
                                    }

                                    if ((A = S[R--]).x === G) {
                                        if (A.y >= 1) {
                                            S[++R] = K(b, A.c, A.l, q, A.z, w, null, 1);

                                            try {
                                                if (["number"].indexOf(typeof S.R) < 0) {
                                                    console.log("_Format_203_S[++R]", JSON.stringify(S[R - 1]));
                                                }
                                            } catch (e) {
                                            }
                                        } else {
                                            S[++R] = K(b, A.c, A.l, q, A.z, w, null, 0);

                                            try {
                                                if (["number"].indexOf(typeof S.R) < 0) {
                                                    console.log("_Format_204_S[++R]", JSON.stringify(S[R - 1]));
                                                }
                                            } catch (e) {
                                            }

                                            A.y++;
                                        }
                                    } else {
                                        S[++R] = A.apply(w, q);

                                        try {
                                            if (["number"].indexOf(typeof S.R) < 0) {
                                                console.log("_Format_205_S[++R]", JSON.stringify(S[R - 1]));
                                            }
                                        } catch (e) {
                                        }
                                    }
                                } else {
                                    if (A > 5) {
                                        C = S[R--];

                                        try {
                                            if (["number"].indexOf(typeof C) < 0) {
                                                console.log("_Format_206_C", JSON.stringify(C));
                                            }
                                        } catch (e) {
                                        }

                                        S[R] = S[R] != C;

                                        try {
                                            if (["number"].indexOf(typeof S[R]) < 0) {
                                                console.log("_Format_207_S[R]", JSON.stringify(S[R]));
                                            }
                                        } catch (e) {
                                        }
                                    } else {
                                        if (A > 3) {
                                            C = S[R--];

                                            try {
                                                if (["number"].indexOf(typeof C) < 0) {
                                                    console.log("_Format_208_C", JSON.stringify(C));
                                                }
                                            } catch (e) {
                                            }

                                            S[R] = S[R] * C;

                                            try {
                                                if (["number"].indexOf(typeof S[R]) < 0) {
                                                    console.log("_Format_209_S[R]", JSON.stringify(S[R]));
                                                }
                                            } catch (e) {
                                            }
                                        } else {
                                            if (A > -1) {
                                                return [1, S[R--]];
                                            }
                                        }
                                    }
                                }
                            }
                        } else {
                            if (A < 2) {
                                if ((A = x) < 4) {
                                    q = S[R--];

                                    try {
                                        if (["number"].indexOf(typeof q) < 0) {
                                            console.log("_Format_210_q", JSON.stringify(q));
                                        }
                                    } catch (e) {
                                    }

                                    if ((A = S[R]).x === G) {
                                        if (A.y >= 1) {
                                            S[R] = K(b, A.c, A.l, [q], A.z, w, null, 1);

                                            try {
                                                if (["number"].indexOf(typeof S[R]) < 0) {
                                                    console.log("_Format_211_S[R]", JSON.stringify(S[R]));
                                                }
                                            } catch (e) {
                                            }
                                        } else {
                                            S[R] = K(b, A.c, A.l, [q], A.z, w, null, 0);

                                            try {
                                                if (["number"].indexOf(typeof S[R]) < 0) {
                                                    console.log("_Format_212_S[R]", JSON.stringify(S[R]));
                                                }
                                            } catch (e) {
                                            }

                                            A.y++;
                                        }
                                    } else {
                                        S[R] = A(q);

                                        try {
                                            if (["number"].indexOf(typeof S[R]) < 0) {
                                                console.log("_Format_213_S[R]", JSON.stringify(S[R]));
                                            }
                                        } catch (e) {
                                        }
                                    }
                                } else {
                                    if (A < 6) {
                                        S[R -= 1] = S[R][S[R + 1]];
                                    } else {
                                        if (A < 8) {
                                            S[R] = --S[R];

                                            try {
                                                if (["number"].indexOf(typeof S[R]) < 0) {
                                                    console.log("_Format_214_S[R]", JSON.stringify(S[R]));
                                                }
                                            } catch (e) {
                                            }
                                        } else {
                                            if (A < 10) {
                                                C = S[R--];

                                                try {
                                                    if (["number"].indexOf(typeof C) < 0) {
                                                        console.log("_Format_215_C", JSON.stringify(C));
                                                    }
                                                } catch (e) {
                                                }

                                                S[R] = typeof C;

                                                try {
                                                    if (["number"].indexOf(typeof S[R]) < 0) {
                                                        console.log("_Format_216_S[R]", JSON.stringify(S[R]));
                                                    }
                                                } catch (e) {
                                                }
                                            }
                                        }
                                    }
                                }
                            } else {
                                if (A < 3) {
                                    if ((A = x) > 11) {
                                        throw S[R--];
                                    }

                                    if (A > 7) {
                                        C = S[R--];

                                        try {
                                            if (["number"].indexOf(typeof C) < 0) {
                                                console.log("_Format_217_C", JSON.stringify(C));
                                            }
                                        } catch (e) {
                                        }

                                        z = W[O];

                                        try {
                                            if (["number"].indexOf(typeof z) < 0) {
                                                console.log("_Format_218_z", JSON.stringify(z));
                                            }
                                        } catch (e) {
                                        }

                                        A = "";

                                        try {
                                            if (["number"].indexOf(typeof A) < 0) {
                                                console.log("_Format_219_A", JSON.stringify(A));
                                            }
                                        } catch (e) {
                                        }

                                        P = i.q[z][0];

                                        try {
                                            if (["number"].indexOf(typeof P) < 0) {
                                                console.log("_Format_220_P", JSON.stringify(P));
                                            }
                                        } catch (e) {
                                        }

                                        for (; P < i.q[z][1]; P++) {
                                            A += String.fromCharCode(r ^ i.p[P]);
                                        }

                                        try {
                                            if (["number"].indexOf(typeof A) < 0) {
                                                console.log("_Format_221_A", JSON.stringify(A));
                                            }
                                        } catch (e) {
                                        }

                                        O += 4;

                                        try {
                                            if (["number"].indexOf(typeof O) < 0) {
                                                console.log("_Format_222_O", JSON.stringify(O));
                                            }
                                        } catch (e) {
                                        }

                                        S[R--][A] = C;
                                    } else {
                                        if (A > 5) {
                                            S[R] = h(S[R]);

                                            try {
                                                if (["number"].indexOf(typeof S[R]) < 0) {
                                                    console.log("_Format_223_S[R]", JSON.stringify(S[R]));
                                                }
                                            } catch (e) {
                                            }
                                        }
                                    }
                                } else {
                                    if ((A = x) < 1) {
                                        S[++R] = null;

                                        try {
                                            if (["number"].indexOf(typeof S.R) < 0) {
                                                console.log("_Format_224_S[++R]", JSON.stringify(S[R - 1]));
                                            }
                                        } catch (e) {
                                        }
                                    } else {
                                        if (A < 3) {
                                            C = S[R--];

                                            try {
                                                if (["number"].indexOf(typeof C) < 0) {
                                                    console.log("_Format_225_C", JSON.stringify(C));
                                                }
                                            } catch (e) {
                                            }

                                            S[R] = S[R] >= C;

                                            try {
                                                if (["number"].indexOf(typeof S[R]) < 0) {
                                                    console.log("_Format_226_S[R]", JSON.stringify(S[R]));
                                                }
                                            } catch (e) {
                                            }
                                        } else {
                                            if (A < 12) {
                                                S[++R] = undefined;

                                                try {
                                                    if (["number"].indexOf(typeof S.R) < 0) {
                                                        console.log("_Format_227_S[++R]", JSON.stringify(S[R - 1]));
                                                    }
                                                } catch (e) {
                                                }
                                            }
                                        }
                                    }
                                }
                            }
                        }
                    } else {
                        if (A < 2) {
                            A = 3 & x;

                            try {
                                if (["number"].indexOf(typeof A) < 0) {
                                    console.log("_Format_228_A", JSON.stringify(A));
                                }
                            } catch (e) {
                            }

                            if (x >>= 2, A > 2) {
                                if ((A = x) > 12) {
                                    S[++R] = l;

                                    try {
                                        if (["number"].indexOf(typeof S.R) < 0) {
                                            console.log("_Format_229_S[++R]", JSON.stringify(S[R - 1]));
                                        }
                                    } catch (e) {
                                    }
                                } else {
                                    if (A > 5) {
                                        C = S[R--];

                                        try {
                                            if (["number"].indexOf(typeof C) < 0) {
                                                console.log("_Format_230_C", JSON.stringify(C));
                                            }
                                        } catch (e) {
                                        }

                                        S[R] = S[R] !== C;

                                        try {
                                            if (["number"].indexOf(typeof S[R]) < 0) {
                                                console.log("_Format_231_S[R]", JSON.stringify(S[R]));
                                            }
                                        } catch (e) {
                                        }
                                    } else {
                                        if (A > 3) {
                                            C = S[R--];

                                            try {
                                                if (["number"].indexOf(typeof C) < 0) {
                                                    console.log("_Format_232_C", JSON.stringify(C));
                                                }
                                            } catch (e) {
                                            }

                                            S[R] = S[R] / C;

                                            try {
                                                if (["number"].indexOf(typeof S[R]) < 0) {
                                                    console.log("_Format_233_S[R]", JSON.stringify(S[R]));
                                                }
                                            } catch (e) {
                                            }
                                        } else {
                                            if (A > 1) {
                                                O += 2 * (z = W[O]) - 2;

                                                try {
                                                    if (["number"].indexOf(typeof O) < 0) {
                                                        console.log("_Format_234_O", JSON.stringify(O));
                                                    }
                                                } catch (e) {
                                                }
                                            } else {
                                                if (A > -1) {
                                                    S[R] = !S[R];

                                                    try {
                                                        if (["number"].indexOf(typeof S[R]) < 0) {
                                                            console.log("_Format_235_S[R]", JSON.stringify(S[R]));
                                                        }
                                                    } catch (e) {
                                                    }
                                                }
                                            }
                                        }
                                    }
                                }
                            } else {
                                if (A > 1) {
                                    if ((A = x) < 2) {
                                        S[++R] = C;

                                        try {
                                            if (["number"].indexOf(typeof S.R) < 0) {
                                                console.log("_Format_236_S[++R]", JSON.stringify(S[R - 1]));
                                            }
                                        } catch (e) {
                                        }
                                    } else {
                                        if (A < 4) {
                                            C = S[R--];

                                            try {
                                                if (["number"].indexOf(typeof C) < 0) {
                                                    console.log("_Format_237_C", JSON.stringify(C));
                                                }
                                            } catch (e) {
                                            }

                                            S[R] = S[R] <= C;

                                            try {
                                                if (["number"].indexOf(typeof S[R]) < 0) {
                                                    console.log("_Format_238_S[R]", JSON.stringify(S[R]));
                                                }
                                            } catch (e) {
                                            }
                                        } else {
                                            if (A < 11) {
                                                C = S[R -= 2][S[R + 1]] = S[R + 2];

                                                try {
                                                    if (["number"].indexOf(typeof C) < 0) {
                                                        console.log("_Format_239_C", JSON.stringify(C));
                                                    }
                                                } catch (e) {
                                                }

                                                R--;
                                            } else {
                                                if (A < 13) {
                                                    C = S[R];

                                                    try {
                                                        if (["number"].indexOf(typeof C) < 0) {
                                                            console.log("_Format_240_C", JSON.stringify(C));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    S[++R] = C;

                                                    try {
                                                        if (["number"].indexOf(typeof S.R) < 0) {
                                                            console.log("_Format_241_S[++R]", JSON.stringify(S[R - 1]));
                                                        }
                                                    } catch (e) {
                                                    }
                                                }
                                            }
                                        }
                                    }
                                } else {
                                    if (A > 0) {
                                        if ((A = x) < 8) {
                                            q = S[R--];

                                            try {
                                                if (["number"].indexOf(typeof q) < 0) {
                                                    console.log("_Format_242_q", JSON.stringify(q));
                                                }
                                            } catch (e) {
                                            }

                                            C = delete S[R--][q];

                                            try {
                                                if (["number"].indexOf(typeof C) < 0) {
                                                    console.log("_Format_243_C", JSON.stringify(C));
                                                }
                                            } catch (e) {
                                            }
                                        } else {
                                            if (A < 10) {
                                                z = W[O];

                                                try {
                                                    if (["number"].indexOf(typeof z) < 0) {
                                                        console.log("_Format_244_z", JSON.stringify(z));
                                                    }
                                                } catch (e) {
                                                }

                                                A = "";

                                                try {
                                                    if (["number"].indexOf(typeof A) < 0) {
                                                        console.log("_Format_245_A", JSON.stringify(A));
                                                    }
                                                } catch (e) {
                                                }

                                                P = i.q[z][0];

                                                try {
                                                    if (["number"].indexOf(typeof P) < 0) {
                                                        console.log("_Format_246_P", JSON.stringify(P));
                                                    }
                                                } catch (e) {
                                                }

                                                for (; P < i.q[z][1]; P++) {
                                                    A += String.fromCharCode(r ^ i.p[P]);
                                                }

                                                try {
                                                    if (["number"].indexOf(typeof A) < 0) {
                                                        console.log("_Format_247_A", JSON.stringify(A));
                                                    }
                                                } catch (e) {
                                                }

                                                O += 4;

                                                try {
                                                    if (["number"].indexOf(typeof O) < 0) {
                                                        console.log("_Format_248_O", JSON.stringify(O));
                                                    }
                                                } catch (e) {
                                                }

                                                S[R] = S[R][A];

                                                try {
                                                    if (["number"].indexOf(typeof S[R]) < 0) {
                                                        console.log("_Format_249_S[R]", JSON.stringify(S[R]));
                                                    }
                                                } catch (e) {
                                                }
                                            } else {
                                                if (A < 12) {
                                                    C = S[R--];

                                                    try {
                                                        if (["number"].indexOf(typeof C) < 0) {
                                                            console.log("_Format_250_C", JSON.stringify(C));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    S[R] = S[R] << C;

                                                    try {
                                                        if (["number"].indexOf(typeof S[R]) < 0) {
                                                            console.log("_Format_251_S[R]", JSON.stringify(S[R]));
                                                        }
                                                    } catch (e) {
                                                    }
                                                } else {
                                                    if (A < 14) {
                                                        S[++R] = W[O];

                                                        try {
                                                            if (["number"].indexOf(typeof S.R) < 0) {
                                                                console.log("_Format_252_S[++R]", JSON.stringify(S[R - 1]));
                                                            }
                                                        } catch (e) {
                                                        }

                                                        O += 2;

                                                        try {
                                                            if (["number"].indexOf(typeof O) < 0) {
                                                                console.log("_Format_253_O", JSON.stringify(O));
                                                            }
                                                        } catch (e) {
                                                        }
                                                    }
                                                }
                                            }
                                        }
                                    } else {
                                        if ((A = x) < 5) {
                                            z = W[O];

                                            try {
                                                if (["number"].indexOf(typeof z) < 0) {
                                                    console.log("_Format_254_z", JSON.stringify(z));
                                                }
                                            } catch (e) {
                                            }

                                            try {
                                                if (t[o][2] = 1, 1 == (C = G(b, O + 4, z - 3, [], c, l, null, 0))[0]) {
                                                    return C;
                                                }
                                            } catch (m) {
                                                if (t[o] && t[o][1] && 1 == (C = G(b, t[o][1][0], t[o][1][1], [], c, l, m, 0))[0]) {
                                                    return C;
                                                }
                                            } finally {
                                                if (t[o] && t[o][0] && 1 == (C = G(b, t[o][0][0], t[o][0][1], [], c, l, null, 0))[0]) {
                                                    return C;
                                                }

                                                t[o] = 0;

                                                try {
                                                    if (["number"].indexOf(typeof t[o]) < 0) {
                                                        console.log("_Format_255_t[o]", JSON.stringify(t[o]));
                                                    }
                                                } catch (e) {
                                                }

                                                o--;
                                            }

                                            O += 2 * z - 2;

                                            try {
                                                if (["number"].indexOf(typeof O) < 0) {
                                                    console.log("_Format_256_O", JSON.stringify(O));
                                                }
                                            } catch (e) {
                                            }
                                        } else {
                                            if (A < 7) {
                                                z = W[O];

                                                try {
                                                    if (["number"].indexOf(typeof z) < 0) {
                                                        console.log("_Format_257_z", JSON.stringify(z));
                                                    }
                                                } catch (e) {
                                                }

                                                O += 2;

                                                try {
                                                    if (["number"].indexOf(typeof O) < 0) {
                                                        console.log("_Format_258_O", JSON.stringify(O));
                                                    }
                                                } catch (e) {
                                                }

                                                S[R -= z] = 0 === z ? new S[R]() : d(S[R], n(S.slice(R + 1, R + z + 1)));
                                            } else {
                                                if (A < 9) {
                                                    C = S[R--];

                                                    try {
                                                        if (["number"].indexOf(typeof C) < 0) {
                                                            console.log("_Format_259_C", JSON.stringify(C));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    S[R] = S[R] & C;

                                                    try {
                                                        if (["number"].indexOf(typeof S[R]) < 0) {
                                                            console.log("_Format_260_S[R]", JSON.stringify(S[R]));
                                                        }
                                                    } catch (e) {
                                                    }
                                                }
                                            }
                                        }
                                    }
                                }
                            }
                        } else {
                            if (A < 3) {
                                A = 3 & x;

                                try {
                                    if (["number"].indexOf(typeof A) < 0) {
                                        console.log("_Format_261_A", JSON.stringify(A));
                                    }
                                } catch (e) {
                                }

                                if (x >>= 2, A < 1) {
                                    if ((A = x) < 2) {
                                        C = S[R--];

                                        try {
                                            if (["number"].indexOf(typeof C) < 0) {
                                                console.log("_Format_262_C", JSON.stringify(C));
                                            }
                                        } catch (e) {
                                        }

                                        S[R] = S[R] > C;

                                        try {
                                            if (["number"].indexOf(typeof S[R]) < 0) {
                                                console.log("_Format_263_S[R]", JSON.stringify(S[R]));
                                            }
                                        } catch (e) {
                                        }
                                    } else {
                                        if (A < 9) {
                                            z = W[O];

                                            try {
                                                if (["number"].indexOf(typeof z) < 0) {
                                                    console.log("_Format_264_z", JSON.stringify(z));
                                                }
                                            } catch (e) {
                                            }

                                            O += 4;

                                            try {
                                                if (["number"].indexOf(typeof O) < 0) {
                                                    console.log("_Format_265_O", JSON.stringify(O));
                                                }
                                            } catch (e) {
                                            }

                                            q = R + 1;

                                            try {
                                                if (["number"].indexOf(typeof q) < 0) {
                                                    console.log("_Format_266_q", JSON.stringify(q));
                                                }
                                            } catch (e) {
                                            }

                                            S[R -= z - 1] = z ? S.slice(R, q) : [];
                                        } else {
                                            if (A < 11) {
                                                z = W[O];

                                                try {
                                                    if (["number"].indexOf(typeof z) < 0) {
                                                        console.log("_Format_267_z", JSON.stringify(z));
                                                    }
                                                } catch (e) {
                                                }

                                                O += 2;

                                                try {
                                                    if (["number"].indexOf(typeof O) < 0) {
                                                        console.log("_Format_268_O", JSON.stringify(O));
                                                    }
                                                } catch (e) {
                                                }

                                                C = S[R--];

                                                try {
                                                    if (["number"].indexOf(typeof C) < 0) {
                                                        console.log("_Format_269_C", JSON.stringify(C));
                                                    }
                                                } catch (e) {
                                                }

                                                c[z] = C;

                                                try {
                                                    if (["number"].indexOf(typeof c[z]) < 0) {
                                                        console.log("_Format_270_c[z]", JSON.stringify(c[z]));
                                                    }
                                                } catch (e) {
                                                }
                                            } else {
                                                if (A < 13) {
                                                    C = S[R--];

                                                    try {
                                                        if (["number"].indexOf(typeof C) < 0) {
                                                            console.log("_Format_271_C", JSON.stringify(C));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    S[R] = S[R] >> C;

                                                    try {
                                                        if (["number"].indexOf(typeof S[R]) < 0) {
                                                            console.log("_Format_272_S[R]", JSON.stringify(S[R]));
                                                        }
                                                    } catch (e) {
                                                    }
                                                } else {
                                                    if (A < 15) {
                                                        S[++R] = W[O];

                                                        try {
                                                            if (["number"].indexOf(typeof S.R) < 0) {
                                                                console.log("_Format_273_S[++R]", JSON.stringify(S[R - 1]));
                                                            }
                                                        } catch (e) {
                                                        }

                                                        O += 4;

                                                        try {
                                                            if (["number"].indexOf(typeof O) < 0) {
                                                                console.log("_Format_274_O", JSON.stringify(O));
                                                            }
                                                        } catch (e) {
                                                        }
                                                    }
                                                }
                                            }
                                        }
                                    }
                                } else {
                                    if (A < 2) {
                                        if ((A = x) < 1) {
                                            S[++R] = g;

                                            try {
                                                if (["number"].indexOf(typeof S.R) < 0) {
                                                    console.log("_Format_275_S[++R]", JSON.stringify(S[R - 1]));
                                                }
                                            } catch (e) {
                                            }
                                        } else {
                                            if (A < 3) {
                                                C = S[R--];

                                                try {
                                                    if (["number"].indexOf(typeof C) < 0) {
                                                        console.log("_Format_276_C", JSON.stringify(C));
                                                    }
                                                } catch (e) {
                                                }

                                                S[R] = S[R] + C;

                                                try {
                                                    if (["number"].indexOf(typeof S[R]) < 0) {
                                                        console.log("_Format_277_S[R]", JSON.stringify(S[R]));
                                                    }
                                                } catch (e) {
                                                }
                                            } else {
                                                if (A < 5) {
                                                    C = S[R--];

                                                    try {
                                                        if (["number"].indexOf(typeof C) < 0) {
                                                            console.log("_Format_278_C", JSON.stringify(C));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    S[R] = S[R] == C;

                                                    try {
                                                        if (["number"].indexOf(typeof S[R]) < 0) {
                                                            console.log("_Format_279_S[R]", JSON.stringify(S[R]));
                                                        }
                                                    } catch (e) {
                                                    }
                                                } else {
                                                    if (A < 14) {
                                                        C = S[R - 1];

                                                        try {
                                                            if (["number"].indexOf(typeof C) < 0) {
                                                                console.log("_Format_280_C", JSON.stringify(C));
                                                            }
                                                        } catch (e) {
                                                        }

                                                        q = S[R];

                                                        try {
                                                            if (["number"].indexOf(typeof q) < 0) {
                                                                console.log("_Format_281_q", JSON.stringify(q));
                                                            }
                                                        } catch (e) {
                                                        }

                                                        S[++R] = C;

                                                        try {
                                                            if (["number"].indexOf(typeof S.R) < 0) {
                                                                console.log("_Format_282_S[++R]", JSON.stringify(S[R - 1]));
                                                            }
                                                        } catch (e) {
                                                        }

                                                        S[++R] = q;

                                                        try {
                                                            if (["number"].indexOf(typeof S.R) < 0) {
                                                                console.log("_Format_283_S[++R]", JSON.stringify(S[R - 1]));
                                                            }
                                                        } catch (e) {
                                                        }
                                                    }
                                                }
                                            }
                                        }
                                    } else {
                                        if (A < 3) {
                                            if ((A = x) < 2) {
                                                z = W[O];

                                                try {
                                                    if (["number"].indexOf(typeof z) < 0) {
                                                        console.log("_Format_284_z", JSON.stringify(z));
                                                    }
                                                } catch (e) {
                                                }

                                                C = "";

                                                try {
                                                    if (["number"].indexOf(typeof C) < 0) {
                                                        console.log("_Format_285_C", JSON.stringify(C));
                                                    }
                                                } catch (e) {
                                                }

                                                P = i.q[z][0];

                                                try {
                                                    if (["number"].indexOf(typeof P) < 0) {
                                                        console.log("_Format_286_P", JSON.stringify(P));
                                                    }
                                                } catch (e) {
                                                }

                                                for (; P < i.q[z][1]; P++) {
                                                    C += String.fromCharCode(r ^ i.p[P]);
                                                }

                                                try {
                                                    if (["number"].indexOf(typeof C) < 0) {
                                                        console.log("_Format_287_C", JSON.stringify(C));
                                                    }
                                                } catch (e) {
                                                }

                                                S[++R] = C;

                                                try {
                                                    if (["number"].indexOf(typeof S.R) < 0) {
                                                        console.log("_Format_288_S[++R]", JSON.stringify(S[R - 1]));
                                                    }
                                                } catch (e) {
                                                }

                                                O += 4;

                                                try {
                                                    if (["number"].indexOf(typeof O) < 0) {
                                                        console.log("_Format_289_O", JSON.stringify(O));
                                                    }
                                                } catch (e) {
                                                }
                                            } else {
                                                if (A < 4) {
                                                    if (S[R--]) {
                                                        O += 4;

                                                        try {
                                                            if (["number"].indexOf(typeof O) < 0) {
                                                                console.log("_Format_290_O", JSON.stringify(O));
                                                            }
                                                        } catch (e) {
                                                        }
                                                    } else {
                                                        O += 2 * (z = W[O]) - 2;

                                                        try {
                                                            if (["number"].indexOf(typeof O) < 0) {
                                                                console.log("_Format_291_O", JSON.stringify(O));
                                                            }
                                                        } catch (e) {
                                                        }
                                                    }
                                                } else {
                                                    if (A < 6) {
                                                        C = S[R--];

                                                        try {
                                                            if (["number"].indexOf(typeof C) < 0) {
                                                                console.log("_Format_292_C", JSON.stringify(C));
                                                            }
                                                        } catch (e) {
                                                        }

                                                        S[R] = S[R] % C;

                                                        try {
                                                            if (["number"].indexOf(typeof S[R]) < 0) {
                                                                console.log("_Format_293_S[R]", JSON.stringify(S[R]));
                                                            }
                                                        } catch (e) {
                                                        }
                                                    } else {
                                                        if (A < 8) {
                                                            C = S[R--];

                                                            try {
                                                                if (["number"].indexOf(typeof C) < 0) {
                                                                    console.log("_Format_294_C", JSON.stringify(C));
                                                                }
                                                            } catch (e) {
                                                            }

                                                            S[R] = S[R] instanceof C;

                                                            try {
                                                                if (["number"].indexOf(typeof S[R]) < 0) {
                                                                    console.log("_Format_295_S[R]", JSON.stringify(S[R]));
                                                                }
                                                            } catch (e) {
                                                            }
                                                        } else {
                                                            if (A < 15) {
                                                                S[++R] = !1;

                                                                try {
                                                                    if (["number"].indexOf(typeof S.R) < 0) {
                                                                        console.log("_Format_296_S[++R]", JSON.stringify(S[R - 1]));
                                                                    }
                                                                } catch (e) {
                                                                }
                                                            }
                                                        }
                                                    }
                                                }
                                            }
                                        } else {
                                            if ((A = x) > 7) {
                                                C = S[R--];

                                                try {
                                                    if (["number"].indexOf(typeof C) < 0) {
                                                        console.log("_Format_297_C", JSON.stringify(C));
                                                    }
                                                } catch (e) {
                                                }

                                                S[R] = S[R] | C;

                                                try {
                                                    if (["number"].indexOf(typeof S[R]) < 0) {
                                                        console.log("_Format_298_S[R]", JSON.stringify(S[R]));
                                                    }
                                                } catch (e) {
                                                }
                                            } else {
                                                if (A > 5) {
                                                    z = W[O];

                                                    try {
                                                        if (["number"].indexOf(typeof z) < 0) {
                                                            console.log("_Format_299_z", JSON.stringify(z));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    O += 2;

                                                    try {
                                                        if (["number"].indexOf(typeof O) < 0) {
                                                            console.log("_Format_300_O", JSON.stringify(O));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    S[++R] = c["$" + z];

                                                    try {
                                                        if (["number"].indexOf(typeof S.R) < 0) {
                                                            console.log("_Format_301_S[++R]", JSON.stringify(S[R - 1]));
                                                        }
                                                    } catch (e) {
                                                    }
                                                } else {
                                                    if (A > 3) {
                                                        z = W[O];

                                                        try {
                                                            if (["number"].indexOf(typeof z) < 0) {
                                                                console.log("_Format_302_z", JSON.stringify(z));
                                                            }
                                                        } catch (e) {
                                                        }

                                                        if (t[o][0] && !t[o][2]) {
                                                            t[o][1] = [O + 4, z - 3];
                                                        } else {
                                                            t[o++] = [0, [O + 4, z - 3], 0];

                                                            try {
                                                                if (["number"].indexOf(typeof t.o) < 0) {
                                                                    console.log("_Format_303_t[o++]", JSON.stringify(t[o - 1]));
                                                                }
                                                            } catch (e) {
                                                            }
                                                        }

                                                        O += 2 * z - 2;

                                                        try {
                                                            if (["number"].indexOf(typeof O) < 0) {
                                                                console.log("_Format_304_O", JSON.stringify(O));
                                                            }
                                                        } catch (e) {
                                                        }
                                                    }
                                                }
                                            }
                                        }
                                    }
                                }
                            } else {
                                A = 3 & x;

                                try {
                                    if (["number"].indexOf(typeof A) < 0) {
                                        console.log("_Format_305_A", JSON.stringify(A));
                                    }
                                } catch (e) {
                                }

                                if (x >>= 2, A > 2) {
                                    if ((A = x) > 13) {
                                        S[++R] = W[O];

                                        try {
                                            if (["number"].indexOf(typeof S.R) < 0) {
                                                console.log("_Format_306_S[++R]", JSON.stringify(S[R - 1]));
                                            }
                                        } catch (e) {
                                        }

                                        O += 8;

                                        try {
                                            if (["number"].indexOf(typeof O) < 0) {
                                                console.log("_Format_307_O", JSON.stringify(O));
                                            }
                                        } catch (e) {
                                        }
                                    } else {
                                        if (A > 11) {
                                            C = S[R--];

                                            try {
                                                if (["number"].indexOf(typeof C) < 0) {
                                                    console.log("_Format_308_C", JSON.stringify(C));
                                                }
                                            } catch (e) {
                                            }

                                            S[R] = S[R] >>> C;

                                            try {
                                                if (["number"].indexOf(typeof S[R]) < 0) {
                                                    console.log("_Format_309_S[R]", JSON.stringify(S[R]));
                                                }
                                            } catch (e) {
                                            }
                                        } else {
                                            if (A > 9) {
                                                S[++R] = !0;

                                                try {
                                                    if (["number"].indexOf(typeof S.R) < 0) {
                                                        console.log("_Format_310_S[++R]", JSON.stringify(S[R - 1]));
                                                    }
                                                } catch (e) {
                                                }
                                            } else {
                                                if (A > 7) {
                                                    z = W[O];

                                                    try {
                                                        if (["number"].indexOf(typeof z) < 0) {
                                                            console.log("_Format_311_z", JSON.stringify(z));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    O += 2;

                                                    try {
                                                        if (["number"].indexOf(typeof O) < 0) {
                                                            console.log("_Format_312_O", JSON.stringify(O));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    S[R] = S[R][z];

                                                    try {
                                                        if (["number"].indexOf(typeof S[R]) < 0) {
                                                            console.log("_Format_313_S[R]", JSON.stringify(S[R]));
                                                        }
                                                    } catch (e) {
                                                    }
                                                } else {
                                                    if (A > 0) {
                                                        C = S[R--];

                                                        try {
                                                            if (["number"].indexOf(typeof C) < 0) {
                                                                console.log("_Format_314_C", JSON.stringify(C));
                                                            }
                                                        } catch (e) {
                                                        }

                                                        S[R] = S[R] < C;

                                                        try {
                                                            if (["number"].indexOf(typeof S[R]) < 0) {
                                                                console.log("_Format_315_S[R]", JSON.stringify(S[R]));
                                                            }
                                                        } catch (e) {
                                                        }
                                                    }
                                                }
                                            }
                                        }
                                    }
                                } else {
                                    if (A > 1) {
                                        if ((A = x) > 10) {
                                            z = W[O];

                                            try {
                                                if (["number"].indexOf(typeof z) < 0) {
                                                    console.log("_Format_316_z", JSON.stringify(z));
                                                }
                                            } catch (e) {
                                            }

                                            t[++o] = [[O + 4, z - 3], 0, 0];

                                            try {
                                                if (["number"].indexOf(typeof t.o) < 0) {
                                                    console.log("_Format_317_t[++o]", JSON.stringify(t[o - 1]));
                                                }
                                            } catch (e) {
                                            }

                                            O += 2 * z - 2;

                                            try {
                                                if (["number"].indexOf(typeof O) < 0) {
                                                    console.log("_Format_318_O", JSON.stringify(O));
                                                }
                                            } catch (e) {
                                            }
                                        } else {
                                            if (A > 8) {
                                                C = S[R--];

                                                try {
                                                    if (["number"].indexOf(typeof C) < 0) {
                                                        console.log("_Format_319_C", JSON.stringify(C));
                                                    }
                                                } catch (e) {
                                                }

                                                S[R] = S[R] ^ C;

                                                try {
                                                    if (["number"].indexOf(typeof S[R]) < 0) {
                                                        console.log("_Format_320_S[R]", JSON.stringify(S[R]));
                                                    }
                                                } catch (e) {
                                                }
                                            } else {
                                                if (A > 6) {
                                                    C = S[R--];

                                                    try {
                                                        if (["number"].indexOf(typeof C) < 0) {
                                                            console.log("_Format_321_C", JSON.stringify(C));
                                                        }
                                                    } catch (e) {
                                                    }
                                                }
                                            }
                                        }
                                    } else {
                                        if (A > 0) {
                                            if ((A = x) > 7) {
                                                C = S[R--];

                                                try {
                                                    if (["number"].indexOf(typeof C) < 0) {
                                                        console.log("_Format_322_C", JSON.stringify(C));
                                                    }
                                                } catch (e) {
                                                }

                                                S[R] = S[R] in C;

                                                try {
                                                    if (["number"].indexOf(typeof S[R]) < 0) {
                                                        console.log("_Format_323_S[R]", JSON.stringify(S[R]));
                                                    }
                                                } catch (e) {
                                                }
                                            } else {
                                                if (A > 5) {
                                                    S[R] = ++S[R];

                                                    try {
                                                        if (["number"].indexOf(typeof S[R]) < 0) {
                                                            console.log("_Format_324_S[R]", JSON.stringify(S[R]));
                                                        }
                                                    } catch (e) {
                                                    }
                                                } else {
                                                    if (A > 3) {
                                                        z = W[O];

                                                        try {
                                                            if (["number"].indexOf(typeof z) < 0) {
                                                                console.log("_Format_325_z", JSON.stringify(z));
                                                            }
                                                        } catch (e) {
                                                        }

                                                        O += 2;

                                                        try {
                                                            if (["number"].indexOf(typeof O) < 0) {
                                                                console.log("_Format_326_O", JSON.stringify(O));
                                                            }
                                                        } catch (e) {
                                                        }

                                                        C = c[z];

                                                        try {
                                                            if (["number"].indexOf(typeof C) < 0) {
                                                                console.log("_Format_327_C", JSON.stringify(C));
                                                            }
                                                        } catch (e) {
                                                        }

                                                        S[++R] = C;

                                                        try {
                                                            if (["number"].indexOf(typeof S.R) < 0) {
                                                                console.log("_Format_328_S[++R]", JSON.stringify(S[R - 1]));
                                                            }
                                                        } catch (e) {
                                                        }
                                                    } else {
                                                        if (A > 1) {
                                                            D = 0;

                                                            try {
                                                                if (["number"].indexOf(typeof D) < 0) {
                                                                    console.log("_Format_329_D", JSON.stringify(D));
                                                                }
                                                            } catch (e) {
                                                            }

                                                            T = S[R].length;

                                                            try {
                                                                if (["number"].indexOf(typeof T) < 0) {
                                                                    console.log("_Format_330_T", JSON.stringify(T));
                                                                }
                                                            } catch (e) {
                                                            }

                                                            $ = S[R];

                                                            try {
                                                                if (["number"].indexOf(typeof $) < 0) {
                                                                    console.log("_Format_331_$", JSON.stringify($));
                                                                }
                                                            } catch (e) {
                                                            }

                                                            S[++R] = function () {
                                                                var b = D < T;

                                                                if (b) {
                                                                    var e = $[D++];
                                                                    S[++R] = e;
                                                                }

                                                                S[++R] = b;
                                                            };

                                                            try {
                                                                if (["number"].indexOf(typeof S.R) < 0) {
                                                                    console.log("_Format_332_S[++R]", JSON.stringify(S[R - 1]));
                                                                }
                                                            } catch (e) {
                                                            }
                                                        }
                                                    }
                                                }
                                            }
                                        } else {
                                            if ((A = x) < 2) {
                                                z = W[O];

                                                try {
                                                    if (["number"].indexOf(typeof z) < 0) {
                                                        console.log("_Format_333_z", JSON.stringify(z));
                                                    }
                                                } catch (e) {
                                                }

                                                A = "";

                                                try {
                                                    if (["number"].indexOf(typeof A) < 0) {
                                                        console.log("_Format_334_A", JSON.stringify(A));
                                                    }
                                                } catch (e) {
                                                }

                                                P = i.q[z][0];

                                                try {
                                                    if (["number"].indexOf(typeof P) < 0) {
                                                        console.log("_Format_335_P", JSON.stringify(P));
                                                    }
                                                } catch (e) {
                                                }

                                                for (; P < i.q[z][1]; P++) {
                                                    A += String.fromCharCode(r ^ i.p[P]);
                                                }

                                                try {
                                                    if (["number"].indexOf(typeof A) < 0) {
                                                        console.log("_Format_336_A", JSON.stringify(A));
                                                    }
                                                } catch (e) {
                                                }

                                                A = +A;

                                                try {
                                                    if (["number"].indexOf(typeof A) < 0) {
                                                        console.log("_Format_337_A", JSON.stringify(A));
                                                    }
                                                } catch (e) {
                                                }

                                                O += 4;

                                                try {
                                                    if (["number"].indexOf(typeof O) < 0) {
                                                        console.log("_Format_338_O", JSON.stringify(O));
                                                    }
                                                } catch (e) {
                                                }

                                                S[++R] = A;

                                                try {
                                                    if (["number"].indexOf(typeof S.R) < 0) {
                                                        console.log("_Format_339_S[++R]", JSON.stringify(S[R - 1]));
                                                    }
                                                } catch (e) {
                                                }
                                            } else {
                                                if (A < 4) {
                                                    C = S[R--];

                                                    try {
                                                        if (["number"].indexOf(typeof C) < 0) {
                                                            console.log("_Format_340_C", JSON.stringify(C));
                                                        }
                                                    } catch (e) {
                                                    }

                                                    S[R] = S[R] - C;

                                                    try {
                                                        if (["number"].indexOf(typeof S[R]) < 0) {
                                                            console.log("_Format_341_S[R]", JSON.stringify(S[R]));
                                                        }
                                                    } catch (e) {
                                                    }
                                                } else {
                                                    if (A < 6) {
                                                        C = S[R--];

                                                        try {
                                                            if (["number"].indexOf(typeof C) < 0) {
                                                                console.log("_Format_342_C", JSON.stringify(C));
                                                            }
                                                        } catch (e) {
                                                        }

                                                        S[R] = S[R] === C;

                                                        try {
                                                            if (["number"].indexOf(typeof S[R]) < 0) {
                                                                console.log("_Format_343_S[R]", JSON.stringify(S[R]));
                                                            }
                                                        } catch (e) {
                                                        }
                                                    } else {
                                                        if (A < 15) {
                                                            C = S[R];

                                                            try {
                                                                if (["number"].indexOf(typeof C) < 0) {
                                                                    console.log("_Format_344_C", JSON.stringify(C));
                                                                }
                                                            } catch (e) {
                                                            }

                                                            S[R] = S[R - 1];

                                                            try {
                                                                if (["number"].indexOf(typeof S[R]) < 0) {
                                                                    console.log("_Format_345_S[R]", JSON.stringify(S[R]));
                                                                }
                                                            } catch (e) {
                                                            }

                                                            S[R - 1] = C;
                                                        }
                                                    }
                                                }
                                            }
                                        }
                                    }
                                }
                            }
                        }
                    }
                }
            }

            return [0, null];
        }

        function K(b, e, f, a, d, c, n, i) {
            var r;
            var t;

            if (null == c) {
                c = this;

                try {
                    if (["number"].indexOf(typeof c) < 0) {
                        console.log("_Format_346_c", JSON.stringify(c));
                    }
                } catch (e) {
                }
            }

            if (d && !d.d) {
                d.d = 0;

                try {
                    if (["number"].indexOf(typeof d.d) < 0) {
                        console.log("_Format_347_d.d", JSON.stringify(d.d));
                    }
                } catch (e) {
                }

                debugger;
                d.$0 = d;

                try {
                    if (["number"].indexOf(typeof d.$0) < 0) {
                        console.log("_Format_348_d.$0", JSON.stringify(d.$0));
                    }
                } catch (e) {
                }

                d[1] = {};
            }

            var o = {};
            var l = o.d = d ? d.d + 1 : 0;
            o["$" + l] = o;
            t = 0;

            try {
                if (["number"].indexOf(typeof t) < 0) {
                    console.log("_Format_349_t", JSON.stringify(t));
                }
            } catch (e) {
            }

            for (; t < l; t++) {
                o[r = "$" + t] = d[r];
            }

            t = 0;

            try {
                if (["number"].indexOf(typeof t) < 0) {
                    console.log("_Format_350_t", JSON.stringify(t));
                }
            } catch (e) {
            }

            l = o.length = a.length;

            try {
                if (["number"].indexOf(typeof l) < 0) {
                    console.log("_Format_351_l", JSON.stringify(l));
                }
            } catch (e) {
            }

            for (; t < l; t++) {
                o[t] = a[t];

                try {
                    if (["number"].indexOf(typeof o[t]) < 0) {
                        console.log("_Format_352_o[t]", JSON.stringify(o[t]));
                    }
                } catch (e) {
                }
            }

            if (i && !B[e]) {
                F(b, e, 2 * f);
            }

            if (B[e]) {
                return G(b, e, f, 0, o, c, null, 1)[1];
            } else {
                return G(b, e, f, 0, o, c, null, 0)[1];
            }
        }
    }, (glb = "undefined" == typeof window ? global : window)._$jsvmprt("",
        [,
            ,
            undefined, //"undefined" != typeof exports ? exports : undefined,
            undefined, //"undefined" != typeof module ? module : undefined,
            "undefined" != typeof define ? define : undefined,
            "undefined" != typeof Object ? Object : undefined,
            undefined,
            "undefined" != typeof TypeError ? TypeError : undefined,
            "undefined" != typeof document ? document : undefined,
            "undefined" != typeof InstallTrigger ? InstallTrigger : undefined,
            "undefined" != typeof safari ? safari : undefined,
            "undefined" != typeof Date ? Date : undefined,
            "undefined" != typeof Math ? Math : undefined,
            window2.navigator, //"undefined" != typeof navigator ? navigator : undefined,
            "undefined" != typeof location ? location : undefined,
            "undefined" != typeof history ? history : undefined,
            "undefined" != typeof Image ? Image : undefined,
            "undefined" != typeof console ? console : undefined,
            "undefined" != typeof PluginArray ? PluginArray : undefined,
            {}, //"undefined" != typeof indexedDB ? indexedDB : undefined,
            "undefined" != typeof DOMException ? DOMException : undefined,
            "undefined" != typeof parseInt ? parseInt : undefined,
            "undefined" != typeof String ? String : undefined,
            "undefined" != typeof Array ? Array : undefined,
            "undefined" != typeof Error ? Error : undefined,
            "undefined" != typeof JSON ? JSON : undefined,
            "undefined" != typeof Promise ? Promise : undefined,
            "undefined" != typeof WebSocket ? WebSocket : undefined,
            "undefined" != typeof eval ? eval : undefined,
            "undefined" != typeof setTimeout ? setTimeout : undefined,
            "undefined" != typeof encodeURIComponent ? encodeURIComponent : undefined,
            "undefined" != typeof encodeURI ? encodeURI : undefined,
            window2["Request"], //"undefined" != typeof Request ? Request : undefined,
            "undefined" != typeof Headers ? Headers : undefined,
            "undefined" != typeof decodeURIComponent ? decodeURIComponent : undefined,
            "undefined" != typeof RegExp ? RegExp : undefined]);
}

delete document.cookie;
delete document.referrer

console.log("------------------\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n-------------------------");
global.byted_acrawler.init({
    "aid": 6383,
    "dfp": false,
    "boe": false,
    "intercept": true,
    "enablePathList": ["/aweme/v1/*", "/aweme/v2/*"]
})
debugger;
console.log("------------------\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n-------------------------");

var xhr = new window.XMLHttpRequest()
xhr.onreadystatechange = function () {
    if (xhr.readyState === 4) {
        console.log("------------------\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n-------------------------");
        console.warn(xhr.status, xhr.statusText, xhr.responseText);
    }
};
console.log("------------------\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n-------------------------");

// 作品列表接口
xhr.open("GET", "/aweme/v1/web/aweme/post/?device_platform=webapp&aid=6383&channel=channel_pc_web&sec_user_id=MS4wLjABAAAAYsGlaAlC63EwxXFJX-DkIdIcMNr5PxZy3h3PvIGobBFp8S11I-0xQMQ2i-Z182vk&max_cursor=1619930039000&count=10&publish_video_strategy_type=2&version_code=160100&version_name=16.1.0", true)

// 喜欢接口
// xhr.open("GET", "/aweme/v1/web/aweme/favorite/?device_platform=webapp&aid=6383&channel=channel_pc_web&sec_user_id=MS4wLjABAAAAYsGlaAlC63EwxXFJX-DkIdIcMNr5PxZy3h3PvIGobBFp8S11I-0xQMQ2i-Z182vk&max_cursor=0&min_cursor=0&count=10&publish_video_strategy_type=2&version_code=160100&version_name=16.1.0", true)

console.log("------------------\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n-------------------------");
xhr.send()
console.log("------------------\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n-------------------------");
debugger;

